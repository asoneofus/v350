...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	fix->line_length = msm_fb_line_length(mfd->index, panel_info->xres,
					      bpp);
	printk(KERN_ERR "fix->line_length(%d)\n",fix->line_length);
	...
	printk(KERN_ERR "1.fix->smem_len(%x),PAGE_SIZE(%lu)\n",fix->smem_len,PAGE_SIZE);
/*
	fix->smem_len = roundup(MAX(msm_fb_line_length(mfd->index,
					       panel_info->xres,
					       bpp) *
			    ...
			    panel_info->mode2_yres * mfd->fb_page), PAGE_SIZE);
	printk(KERN_ERR "2.fix->smem_len(%x),PAGE_SIZE(%lu)\n",fix->smem_len,PAGE_SIZE);
*/
	...
	if (fbram_size < fix->smem_len) {
		printk(KERN_ERR "%s():error: no more framebuffer memory!(%x,%x)\n",__func__,fbram_size,fix->smem_len);
		return -ENOMEM;
	}else{
		printk(KERN_ERR "%s(): framebuffer memory!(%x,%x)\n",__func__,fbram_size,fix->smem_len);
	}
