...
do { \
    ...
    if(ret) fih_printk(debug_mask_lcm, FIH_LCM_ERR, KERN_ERR "[lcdc_ili9338] SPI write error  0x%x.\n", data);\
} while(0)
...
static void reset_lcmic(void)
{
    ...
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcdc_ili9338: reset_lcmic(): gpio_direction_output hight: read(%d)\n", gpio_get_value(gpio_lcd_reset));
    mdelay(1);

    ret = gpio_direction_output(gpio_lcd_reset, 0);
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcdc_ili9338: reset_lcmic(): gpio_direction_output low: read(%d)\n", gpio_get_value(gpio_lcd_reset));
    mdelay(10);
...
static int ilitek_config(struct driver_data *dd, struct spi_device *spi)
{
    int ret=0;
    reset_lcmic();
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcdc_ili9338: ilitek_config()\n");
    //************* Start Initial Sequence **********//
    LCD_ILI9481_CMD(0x11);    
    Delayms(20);
    
    //lcdc_ili9338: for polarity of den, hsync, vsync, pclk
...
//    LCD_ILI9481_INDEX(0x02); 
    ...
    */
    ...
    //lcdc_ili9338: Display timing (form innolux) 
    WriteI(0xC1);
    WriteD(0x10);  
    WriteD(0x10); 
    WriteD(0x88);
...
inline int lcdc_ili9338_config_v1(struct driver_data *dd, struct spi_device *spi)
{
	int ret=0;
	/*
	NRES = 1;
	...
	*/
	...
	WriteI(0x11); //Exit Sleep
	Delayms(120);

	WriteI(0x3A); //Set Pixel format is 18bit/pixel.   
	WriteD(0x66);

	WriteI(0xCB); //AP[2:0]
	WriteD(0x01);

	WriteI(0xC0); //Power control
	WriteD(0x22); //VRH[5:0]            
	WriteD(0x00); //VCI1[2:0]

	WriteI(0xC1); //Power control
	WriteD(0x11); //SAP[2:0];BT[3:0] 
    ...
    WriteI(0xB0);   
    WriteD(0x03); //change polarity of DE, DOTCLK
	...
	WriteD(0x27);
	//========================================//       
	WriteI(0xE0);   //Set Gamma
	WriteD(0x0D);    //VP63[3:0]0
	WriteD(0x31);    //VP62[5:0]
	WriteD(0x25);    //VP61[5:0]
	WriteD(0x0C);    //VP59[3:0]
	WriteD(0x0F);    //VP57[4:0]
	WriteD(0x08);    //VP50[3:0]
	WriteD(0x55);    //VP43[6:0]
	WriteD(0x89);    //VP36[3:0] VP27[3:0]
	WriteD(0x45);    //VP20[6:0]
	WriteD(0x0B);    //VP13[3:0]   
	WriteD(0x16);    //VP6[4:0]
	WriteD(0x08);    //VP4[3:0]
	WriteD(0x20);    //VP2[5:0]
	WriteD(0x16);    //VP1[5:0]
	...
	WriteI(0xE1);   //Set Gamma
	WriteD(0x02);    //VN63[3:0]
	WriteD(0x0E);    //VN62[5:0]
	WriteD(0x1A);    //VN61[5:0]
	WriteD(0x03);    //VN59[3:0]
	WriteD(0x10);    //VN57[4:0]
	WriteD(0x07);    //VN50[3:0]
	WriteD(0x2A);    //VN43[6:0]
	WriteD(0x67);    //VN36[3:0] VN27[3:0]
	WriteD(0x3A);    //VN20[6:0]
	WriteD(0x04);    //VN13[3:0]
	WriteD(0x09);    //VN6[4:0]
	WriteD(0x07);    //VN4[3:0]
	WriteD(0x1F);    //VN2[5:0]
	WriteD(0x29);    //VN1[5:0]
	...
	WriteI(0x2A); //CASET (Column Address Set)
	WriteD(0x00); 
	WriteD(0x00); 
	WriteD(0x00); 
	WriteD(0xEF); 

	WriteI(0x2B); //PASET (Page Address Set)
	WriteD(0x00); 
	WriteD(0x00); 
	WriteD(0x01); 
	WriteD(0x3F); 

	WriteI(0x29); //display on
	//Delayms(120);  
	return ret;
   
}
inline int lcdc_ili9338_config(struct driver_data *dd, struct spi_device *spi)
{
	return lcdc_ili9338_config_v1(dd,spi);    
#if 0	
    int ret=0;
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcdc_ili9338: %s()\n", __func__);
...
static int simple_config(struct driver_data *dd, struct spi_device *spi)
{
    int ret=0;
    reset_lcmic();
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcdc_ili9338: simple_config()\n");
...
static int get_power_mode(struct driver_data *dd, struct spi_device *spi)
{
    ...
    int ret=0;
    reset_lcmic();
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcdc_ili9338: get_power_mode()\n");
    LCD_ILI9481_CMD(0x11);    
    Delayms(20);
...
/*
    reset_lcmic();
    printk(KERN_INFO "lcdc_ili9338: get_pixel_mode()\n");
...
*/
    ...
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcdc_ili9338: get_pixel_mode(), tx(0x%x), rxbuf[]=[0x%x,0x%x]\n", txbuf, rxbuf[0], rxbuf[1] );
...
static int sleep_config(struct driver_data *dd, struct spi_device *spi)
{
    ...
    reset_lcmic();
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcdc_ili9338: get_pixel_mode()\n");
...
void lcdc_ili9338_panel_off(void)
{
    int ret=0;
    struct spi_device *spi=g_spi;
    
    if (!spi){
        fih_printk(debug_mask_lcm, FIH_LCM_ERR, KERN_INFO "lcdc_ili9338: %s(), spi NULL! do nothing.\n", __func__);    
        return;
    }
...
EXPORT_SYMBOL(lcdc_ili9338_panel_off);
...
void lcdc_ili9338_panel_on(void)
{
    ...
    //chandler_boot_lcdc
	if(panel_first_on==1) {
	    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcdc_ili9338: %s(): skip first panel on\n", __func__);    
    	panel_first_on=0;
        return;
    }
    ...
    reset_lcmic();
    lcdc_ili9338_config(0,spi);
    
    //Delayms(100);
}


EXPORT_SYMBOL(lcdc_ili9338_panel_on);
                ...
                const char *buff, size_t len, loff_t *off)
{
	...
	{
		lcdc_ili9338_config( NULL, g_spi);
...
static int fih_lcm_is_mddi_type(void)
{
	return 0;
}


int fih_lcm_is_ili9338(void)
{
...
static void disable_mddi_clock(void)
{
    ...
    printk(KERN_INFO "lcdc_ili9338: disable_mddi_clock()\n");
}

static int __init lcdc_ili9338_init(void)
{
	int ret;
	struct msm_panel_info pinfo={0};
	//struct vreg *vreg_gp5;	
	
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcdc_ili9338: +%s()\n",__func__);
    
	if(!fih_lcm_is_ili9338()){
        ret=-ENODEV;
		goto err;
	}
	
    if(fih_lcm_is_mddi_type()){
        fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcdc_ili9338: %s(): LCM isn't RGB i/f\n",__func__);
        lcdc_clk_off();        
        ret=-ENODEV;
        goto err;
    }
    ...
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcdc_ili9338: %s(): pinfo.clk_rate(%d)\n", __func__, pinfo.clk_rate);
...
static void __exit lcdc_ili9338_exit(void)
{
    spi_unregister_driver(&lcdc_spi_drv);
}

module_init(lcdc_ili9338_init);
module_exit(lcdc_ili9338_exit);
