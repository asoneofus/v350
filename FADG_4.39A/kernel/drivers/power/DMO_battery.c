...
#define T_FIH
#ifdef T_FIH	//+T_FIH
//MayLi, change for using 7019 GPIO, 2010.10.28 {+
...
//FIHTDC, Add for increasing 10 degree of condition, MayLi, 2011.04.14 {+++
#define BATTERY_TEMP_COOL_DOWN_FROM_EMGCY   60//50  //Battery temp. lower than 50 degree leaves cool down mode.
#define BATTERY_TEMP_SHUTDOWN_AP    65//55  //Battery temperature threshod to shut down AP to cool down device.
#define BATTERY_TEMP_EMGCY_CALL_ONLY    68//58  //Battery temperature threshod for emergency call only.
//FIHTDC, Add for increasing 10 degree of condition, MayLi, 2011.04.14 ---}
...
//FIHTDC, Add for fast charge issue, MayLi, 2011.06.09 {+++
VOLT_TO_PERCENT g_vtcFastChargeBase;
int g_iTempCapacity=0;
int g_iPreCount=0;
int g_iChargerInOutTime=0;
//FIHTDC, Add for fast charge issue, MayLi, 2011.06.09 ---}
...
{
    {3450, 0},  //{ 3450, 0},	   // empty,    Rx Table
    {3620, 10},  //{ 3599, 10},    // level 1
    {3675, 20},  //{ 3654, 20},    // level 2
    {3700, 30},   //{ 3674, 30},    // level 3
    {3725, 40},  //{ 3708, 40},    // level 4
    {3760, 50},  //{ 3737, 50},    // level 5
    {3810, 60},  //{ 3782, 60},    // level 6
    {3880, 70},  //{ 3839, 70},    // level 7
    {3945, 80},  //{ 3902, 80},    // level 8
    {4020, 90},  //{ 3995, 90},    // level 9
    {4100, 100},  //{ 4100, 100},   // full
};
...
{
  { 3450, 0},   //{ 3450, 0},	   // empty,    Rx Table
  { 3650, 10},   //{ 3562, 10},    // level 1
  { 3700, 20},   //{ 3616, 20},    // level 2
  { 3726, 30},   //{ 3651, 30},    // level 3
  { 3750, 40},   //{ 3678, 40},    // level 4
  { 3785, 50},   //{ 3711, 50},    // level 5
  { 3830, 60},   //{ 3756, 60},    // level 6
  { 3890, 70},   //{ 3818, 70},    // level 7
  { 3955, 80},   //{ 3887, 80},    // level 8
  { 4035, 90},   //{ 3973, 90},    // level 9
  { 4100, 100},   //{ 4100, 100},   // full
};
...
//FIHTDC, Add for fast charge issue, MayLi, 2011.06.09 {+++
static VOLT_TO_PERCENT g_VC_P[11] =
{
    {3450, 0},  //{ 3450, 0},	   // empty,    Rx Table
    {3620, 150},  //{ 3599, 10},    // level 1
    {3675, 300},  //{ 3654, 20},    // level 2
    {3700, 450},   //{ 3674, 30},    // level 3
    {3725, 600},  //{ 3708, 40},    // level 4
    {3760, 750},  //{ 3737, 50},    // level 5
    {3810, 900},  //{ 3782, 60},    // level 6
    {3880, 1050},  //{ 3839, 70},    // level 7
    {3945, 1200},  //{ 3902, 80},    // level 8
    {4020, 1350},  //{ 3995, 90},    // level 9
    {4100, 1500},  //{ 4100, 100},   // full
};

//DQE, DMQ, DQD
static VOLT_TO_PERCENT g_VC_Q[11] =
{
  { 3450, 0},   //{ 3450, 0},	   // empty,    Rx Table
  { 3650, 139},   //{ 3562, 10},    // level 1
  { 3700, 278},   //{ 3616, 20},    // level 2
  { 3726, 417},   //{ 3651, 30},    // level 3
  { 3750, 556},   //{ 3678, 40},    // level 4
  { 3785, 695},   //{ 3711, 50},    // level 5
  { 3830, 834},   //{ 3756, 60},    // level 6
  { 3890, 973},   //{ 3818, 70},    // level 7
  { 3955, 1112},   //{ 3887, 80},    // level 8
  { 4035, 1251},   //{ 3973, 90},    // level 9
  { 4100, 1390},   //{ 4100, 100},   // full
};

//Default table.
static VOLT_TO_PERCENT g_VC_D[11] =
{
    { 3450, 0},	   // empty,    Rx Table 
    { 3556, 150},    // level 1
    { 3642, 300},    // level 2
...
int bat_id_check_TI=0;  //FIHTDC, Add for new battery EPROM chip of maxim, MayLi, 2011.08.31 
...
//FIHTDC, Add for partial charge when over heat, MayLi, 2011.07.14 {++
bool over_temper;  /* 1: disable charge, 0: still charge */
bool over_temper_1=0; /* 1:Enter High or Low temp, 0: save temp */
//FIHTDC, Add for partial charge when over heat, MayLi, 2011.07.14 --}
...
//New charging temperature protection scenario --}
int zeus_bat_vol;
int zeus_bat_average=0;  /*FIHTDC, Add for fast charge issue, MayLi, 2011.06.09*/
...
//FIHTDC, Add for new battery EPROM chip of maxim, MayLi, 2011.08.31 {++
u8 w1_bq2024_read_1_maxim(void)
{
	int ret;
	...
	udelay(15);   /*slave will pull low 17us~60us*/
...
u8 w1_bq2024_read_8_maxim(void)
{
	unsigned long flags;
	int i;
	u8 data=0;
	...
	for (i=0; i<8; i++) 
	{
		data |= ((w1_bq2024_read_1_maxim() & 0x1) << i);
	}
	spin_unlock_irqrestore(&bq2024_spin_lock, flags);
	return data;
}

u8 w1_bq2024_read_1_TI(void)
{
	int ret;
...
u8 w1_bq2024_read_8_TI(void)
{
	...
	for (i=0; i<8; i++) 
	{
		data |= ((w1_bq2024_read_1_TI() & 0x1) << i);
	}
	spin_unlock_irqrestore(&bq2024_spin_lock, flags);
	return data;
}
...
u8 Check_CRC8(u8 * data, int len)
{
	...
	//FIHTDC, Add for new battery EPROM chip of maxim, MayLi, 2011.08.31 {++
	if(bat_id_check_TI)
		bq2024_crc = w1_bq2024_read_8_TI();
	else
		bq2024_crc = w1_bq2024_read_8_maxim();
	//FIHTDC, Add for new battery EPROM chip of maxim, MayLi, 2011.08.31 --}
...
int w1_reset_select_slave(void)
{
	...
	if(bat_id_check_TI){
		for(i=0; i<7; i++)
		{
			data[i] = w1_bq2024_read_8_TI();
			printk(KERN_ERR "%s: data[%d]=0x%x\r\n", __func__,i,data[i] );
		}
	}else{
		for(i=0; i<7; i++)
		{
			data[i] = w1_bq2024_read_8_maxim();
			printk(KERN_ERR "%s: data[%d]=0x%x\r\n", __func__,i,data[i] );
		}
	}
	//FIHTDC, Add for new battery EPROM chip of maxim, MayLi, 2011.08.31 --}
...
int w1_bq2024_read_page(u8 PNum, u8* PData)
{
	...
	if(bat_id_check_TI){
		for(i=0; i<32; i++)
		{
			TempPData[i] = w1_bq2024_read_8_TI();
		}
	}else{
		for(i=0; i<32; i++)
		{
			TempPData[i] = w1_bq2024_read_8_maxim();
		}
	}
	//FIHTDC, Add for new battery EPROM chip of maxim, MayLi, 2011.08.31 --}
...
int CheckBatteryID(void)
{
	...
	// FIHTDC, Add for check battery id, MayLi, 2011.04.02 { ++
	#if 0
	...
	if( memcmp(&BatteryID_Table[0][96], &EPROM[96], 4) != 0 ||
		memcmp(&BatteryID_Table[0][104], &EPROM[104], 24) != 0)
	{
		printk(KERN_ERR "%s: compare DQE, DMQ, DQD, DPD, DMP, DP2 ID fail !!\r\n", __func__);
		return -1;
	}
	// FIHTDC, Add for check battery id, MayLi, 2011.04.02 --}
...
static int	GetPMIC_MSM_TERMAL(void)
{
	...
	//FIHTDC, Michael add for battery test mode, 2010.12.29---
   	...
   	if((g_Product>=Project_DQE && g_Product<=Project_DP2 && !(g_Product==Project_DMQ &&  g_PR_num==Phase_PR1))
		|| g_Product == Project_AI1D || g_Product == Project_AI1S 
		|| g_Product == Project_DMO || g_Product == Project_DMT) /*Do not use battery thermal when DMQ_PR1, HW issue. */
    	{
    		if(Battery_ID_status)
    		{
    			adc_read_id = 35;
				g_use_battery_thermal = 1;
			}
			else
			{
				adc_read_id = 36;
				g_use_battery_thermal = 0;
			}
    	}
	else
	...
	else//Charging unplug
	{
		...
		for(iC=0;iC<11;iC++)
		{
			if(g_Product == Project_DQE ||
				g_Product == Project_DMQ||
				g_Product == Project_DQD ||
				g_Product == Project_DMO ||
				g_Product == Project_DMT)
			{
		...
		if(g_Product == Project_DQE || g_Product == Project_DMQ|| g_Product == Project_DQD ||
			g_Product == Project_DMO || g_Product == Project_DMT)
...
//FIHTDC, Add for fast charge issue, MayLi, 2011.06.09 {+++
int VoltageToCapacity(unsigned Volt)
{
	int iC;
	int capacity=0;
	...
	for(iC=0;iC<11;iC++)
	{
			...
			g_Product == Project_DMT)
		{
			if(Volt <= g_VC_Q[iC].dwVolt)
				...
				g_Product == Project_AI1S)
		{
			if(Volt <= g_VC_P[iC].dwVolt)
           			break;
		}
		else 
		{
			if(Volt <= g_VC_D[iC].dwVolt)
           			break;
		}
	}
	...
	else if((iC>0)&&(iC<11))
	{
		if(g_Product == Project_DQE || 
			...
			g_Product == Project_DMT)
		{
			capacity=g_VC_Q[iC-1].dwPercent + 
				( Volt -g_VC_Q[iC-1].dwVolt) * ( g_VC_Q[iC].dwPercent -g_VC_Q[iC-1].dwPercent)/( g_VC_Q[iC].dwVolt -g_VC_Q[iC-1].dwVolt);
		}
		else if(g_Product == Project_DPD || 
				g_Product == Project_DMP||
				g_Product == Project_DP2 ||
				...
				g_Product == Project_AI1S)
		{
			capacity=g_VC_P[iC-1].dwPercent + 
				( Volt -g_VC_P[iC-1].dwVolt) * ( g_VC_P[iC].dwPercent -g_VC_P[iC-1].dwPercent)/( g_VC_P[iC].dwVolt -g_VC_P[iC-1].dwVolt);
		}
		else
		{
			capacity=g_VC_D[iC-1].dwPercent + 
				( Volt -g_VC_D[iC-1].dwVolt) * ( g_VC_D[iC].dwPercent -g_VC_D[iC-1].dwPercent)/( g_VC_D[iC].dwVolt -g_VC_D[iC-1].dwVolt);
		}
	}
	fih_printk(battery_debug_mask, FIH_DEBUG_ZONE_G7,  "%s(), volt(%d) -> capacity(%d)\r\n", __func__, Volt, capacity);
	return capacity;
}

int CapacityToVoltage(int Capacity)
{
	int iC; 
	unsigned	volt=0;
	...
	for(iC=0;iC<11;iC++)
	{
		if(g_Product == Project_DQE || 
			...
			g_Product == Project_DMT)
		{
			if(Capacity <= g_VC_Q[iC].dwPercent)
           			break;
		}
		else if(g_Product == Project_DPD || 
				g_Product == Project_DMP||
				...
				g_Product == Project_AI1S)
		{
			if(Capacity <= g_VC_P[iC].dwPercent)
           			break;
		}
		else 
		{
			if(Capacity <= g_VC_D[iC].dwPercent)
           			break;
		}
	}
	...
	else if((iC>0)&&(iC<11))
	{
		if(g_Product == Project_DQE || 
			...
			g_Product == Project_DMT)
		{
			volt = g_VC_Q[iC-1].dwVolt + 
				( Capacity -g_VC_Q[iC-1].dwPercent) * ( g_VC_Q[iC].dwVolt-g_VC_Q[iC-1].dwVolt)/( g_VC_Q[iC].dwPercent -g_VC_Q[iC-1].dwPercent);
		}
		else if(g_Product == Project_DPD || 
				g_Product == Project_DMP||
				g_Product == Project_DP2 ||
				...
				g_Product == Project_AI1S)
		{
			volt = g_VC_P[iC-1].dwVolt + 
				( Capacity -g_VC_P[iC-1].dwPercent) * ( g_VC_P[iC].dwVolt-g_VC_P[iC-1].dwVolt)/( g_VC_P[iC].dwPercent -g_VC_P[iC-1].dwPercent);
		}
		else
		{
			volt = g_VC_D[iC-1].dwVolt + 
				( Capacity -g_VC_D[iC-1].dwPercent) * ( g_VC_D[iC].dwVolt-g_VC_D[iC-1].dwVolt)/( g_VC_D[iC].dwPercent -g_VC_D[iC-1].dwPercent);
		}
	}
	fih_printk(battery_debug_mask, FIH_DEBUG_ZONE_G7,  "%s(), capacity(%d) -> volt(%d)\r\n", __func__, Capacity, volt);
	return volt;
}

//FIHTDC, Add for fix jiffies stop in suspend mode, MayLi, 2011.06.23 {++
#include <linux/rtc-msm.h> /*FIHTDC, Add for check rtc ready, MayLi, 2011.06.23 */
int CapacityCompare(unsigned Volt)
{
	VOLT_TO_PERCENT vtcFastChargeNow={0,0};
	int iCapacityTemp=0;
	int iNowCount=0;
	int iChargeA = 450; 
	...
	if(Charging_current==500)
		iChargeA = 450;
	else if(Charging_current==1000)
	{
		if( g_Product == Project_AI1S || g_Product == Project_AI1D || g_Product == Project_FAD)
			iChargeA = 900;
		else
			iChargeA = 700;
	}
	vtcFastChargeNow.dwVolt = Volt;
	vtcFastChargeNow.dwPercent = VoltageToCapacity(Volt);

	//When charger plug out and table did not change to Idle table,
	//we need to change the voltage that reported to system to g_wTempCapacity.
	//That will avoid battery meter from low to high.

	if(RTC_ready==0)
	{
		RTC_ready = get_rtc_ready();
	}
	
	if(!gpio_get_value(GPIO_CHR_DET) && RTC_ready)
	{       
		if( g_iPreCount ==0)
		{
			if(g_iChargerInOutTime==0)
				g_iPreCount = get_seconds();
			else
				g_iPreCount = g_iChargerInOutTime;

		}
		if(Volt<3450)
			g_iPreCount = get_seconds();

		iNowCount = get_seconds();
		//FIHTDC, rtc reset will let count error, so ignore this time, MayLi, 2011.07.06 {++
		if((iNowCount < g_iPreCount) || (iNowCount-g_iPreCount > 610))
		{
			printk(KERN_INFO "%s(), Timer Reset,  pre(%d), now(%d)!!!\r\n", __func__, g_iPreCount, iNowCount);
			g_iPreCount = iNowCount;
		}
		//FIHTDC, rtc reset will let count error, so ignore this time, MayLi, 2011.07.06 --}
		iCapacityTemp = (iNowCount-g_iPreCount)*iChargeA/3600;
		fih_printk(battery_debug_mask, FIH_DEBUG_ZONE_G7,  "%s(), g_iPreCount=%d, iNowCount=%d\r\n",
															__func__, g_iPreCount, iNowCount);
		g_iTempCapacity += iCapacityTemp;
		g_iPreCount = iNowCount;
	}
	
	fih_printk(battery_debug_mask, FIH_DEBUG_ZONE_G7,  "%s(), nowV(%d), nowC(%d), iCur(%d), TempC(%d)\r\n",
														__func__, vtcFastChargeNow.dwVolt, vtcFastChargeNow.dwPercent,
														iChargeA, g_iTempCapacity);
	if(g_iTempCapacity<vtcFastChargeNow.dwPercent)
		return CapacityToVoltage(g_iTempCapacity);

	return vtcFastChargeNow.dwVolt;
}
//FIHTDC, Add for fast charge issue, MayLi, 2011.06.09 ---}
//FIHTDC, Add for fix jiffies stop in suspend mode, MayLi, 2011.06.23 --}
...
static int	ChangeToVoltPercentage(unsigned Vbat)
{
	...
	fih_printk(battery_debug_mask, FIH_DEBUG_ZONE_G8,  "%s(): %d, %d, %d, %d, %d, %d, %d, %d, %d\r\n", __func__, 
					charging_bat_vol, VBAT_from_SMEM, zeus_bat_average, 
					CapacityToVoltage(g_iTempCapacity), Vbat, Volt_pec, pec_return, 
					gpio_get_value(GPIO_CHR_EN), Charging_current);
	return pec_return;
}
...
void Check_OTP(void)
{
	//Low temp, t<0 or t>=60
	if(msm_termal <= (BATTERY_TEMP_LOW_LIMIT-BATTERY_THERMAL_TEMP_OFFSET) ||
		msm_termal>=60)
	{
	...
	//High temp, 45<t<60
	if(msm_termal >= (BATTERY_TEMP_HIGH_LIMIT+BATTERY_THERMAL_TEMP_OFFSET) 
		&& msm_termal < 60)
	{
		over_temper_1=true;
		...
		if(!((g_Product==Project_DMQ)&&(g_PR_num==Phase_PR1))) /*Do not disable charging. DMQ_PR1 HW issue*/
		{
			if(zeus_bat_vol < 3800)
			{
				printk("%s: Battery OTP partial=%d\n", __func__, msm_termal);
				eventlog("B_OTP:%d, partial\n", msm_termal);	
				over_temper=false;
				SetChargingCurrent(500);
			}
			else
			{
				printk("%s: Battery OTP partial disable=%d\n", __func__, msm_termal);
				eventlog("B_OTP:%d, partial disable\n", msm_termal);	
				over_temper=true;
				if(!((g_Product==Project_DMQ)&&(g_PR_num==Phase_PR1))) /*Do not disable charging. DMQ_PR1 HW issue*/
				{
					if(!gpio_get_value(GPIO_CHR_DET))
				 ...
				 union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_HEALTH:
		...
		if((g_Product >= Project_DQE &&  g_Product <= Project_DP2) || g_Product == Project_DMO || g_Product == Project_DMT)
		{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		else
		{
			...
			//FIHTDC, Add for fast charge issue, MayLi, 2011.06.09 {+++
			zeus_bat_average = vbatt;
			if(zeus_bat_val<100)
				vbatt = CapacityCompare(vbatt);
			//FIHTDC, Add for fast charge issue, MayLi, 2011.06.09 ---}
			...
			{
				if(!over_temper&&((msm_termal<=0)||(msm_termal>=48)))
				{
					printk("%s: MSM OTP=%d, discharge \n", __func__, msm_termal);
					eventlog("M_OTP:%d, dis-charge\n", msm_termal);
				...
				if(over_temper&&((msm_termal>=5)&&(msm_termal<=43)))
				{
...
static irqreturn_t chgdet_irqhandler(int irq, void *dev_id)
{
	...
	if(g_charging_state == CHARGER_STATE_NOT_CHARGING)
	{
		...
		/*FIHTDC, Add for fast charge issue, MayLi, 2011.06.09 {+++*/
		g_iChargerInOutTime = 0; 
		//When charger plug out, record the FastchargeBase
		if(g_vtcFastChargeBase.dwVolt!=0)
		{
			g_vtcFastChargeBase.dwPercent= g_iTempCapacity;
			g_vtcFastChargeBase.dwVolt = CapacityToVoltage(g_iTempCapacity);
			g_iPreCount=0;
		}
		/*FIHTDC, Add for fast charge issue, MayLi, 2011.06.09 ---}*/
	}
	else
	{
		...
		if(g_Product == Project_DPD ||
			...
			g_Product == Project_DMP ||
			g_Product == Project_DQD ||
			(g_Product == Project_DMO && g_PR_num>=Phase_MP) || /*FITHDC, Add for check DMO battery id after MP, MayLi, 2011.08.31 */
			g_Product == Project_IRF)
		{	
			//FIHTDC, Add for new battery EPROM chip of maxim, MayLi, 2011.08.31 {++
			bat_id_check_TI = 0;
			if(CheckBatteryID() != 0)
		...
		//FIHTDC, Add for fix jiffies stop in suspend mode, MayLi, 2011.06.23 {++
		if(get_rtc_ready())
			g_iChargerInOutTime = get_seconds();
		//FIHTDC, Add for fix jiffies stop in suspend mode, MayLi, 2011.06.23 --}
		//When charger plug in, set the FastChargeBase
		if(g_vtcFastChargeBase.dwVolt==0 ||g_vtcFastChargeBase.dwVolt>zeus_bat_vol)
			g_vtcFastChargeBase.dwVolt=zeus_bat_vol;
		g_vtcFastChargeBase.dwPercent = VoltageToCapacity(g_vtcFastChargeBase.dwVolt);                     
		g_iTempCapacity = g_vtcFastChargeBase.dwPercent;
		/*FIHTDC, Add for fast charge issue, MayLi, 2011.06.09 ---}*/
	}
...
/*	data->ac.properties = goldfish_ac_props;
	...
	//FIHTDC, add for fast charging issue, MayLi, 2011.06.09 {++
	//zeus_bat_vol=0;
	zeus_bat_vol = GetPMICBatteryInfo();
	if(g_vtcFastChargeBase.dwVolt==0 ||g_vtcFastChargeBase.dwVolt>zeus_bat_vol)
		g_vtcFastChargeBase.dwVolt=zeus_bat_vol;
	g_vtcFastChargeBase.dwPercent = VoltageToCapacity(g_vtcFastChargeBase.dwVolt);                     
	g_iTempCapacity = g_vtcFastChargeBase.dwPercent;
	printk("%s(): zeus_bat_vol=%d\n", __func__,  zeus_bat_vol);
	//FIHTDC, add for fast charging issue, MayLi, 2011.06.09 --}
	...
	if((g_Product>=Project_DQE && g_Product<=Project_DP2 && !(g_Product==Project_DMQ &&  g_PR_num==Phase_PR1))
		|| g_Product == Project_DMO || g_Product == Project_DMT 
		|| g_Product == Project_AI1D || g_Product == Project_AI1S) /*Do not use battery thermal when DMQ_PR1, HW issue. */
	{
		g_use_battery_thermal = TRUE;
		printk("%s(): Battery thermistor exists\n", __func__);
	}
