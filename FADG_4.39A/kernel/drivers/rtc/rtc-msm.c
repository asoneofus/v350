...
//Michael add for Power off alarm+++
static ssize_t RTCAlarmSettime(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
	unsigned time_set;
	...
	sscanf(buf, "%u\n", &time_set);
	time_set=time_set*1000;//to ,mili-second
	
	printk(KERN_INFO "%s: %u\n", __func__, time_set);

	msm_proc_comm_oem(PCOM_CUSTOMER_CMD1, &oem_cmd, &smem_response, &time_set);
	return count;
}
DEVICE_ATTR(rtcset, 0666, NULL, RTCAlarmSettime);
//Michael add for Power off alarm---

//FIHTDC, Add for check rtc ready, MayLi, 2011.06.23 {++
static int rtc_ready=0;
int get_rtc_ready(void)
{
	return rtc_ready;
}
//FIHTDC, Add for check rtc ready, MayLi, 2011.06.23 --}
...
#endif
	rtc_ready = 1; /* FIHTDC, Add for check rtc ready, MayLi, 2011.06.23 */
...
msmrtc_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	int RPC_wakeup_cycle_time=600;  /*FIHTDC, MayLi adds for battery updates per 10 minutes in suspend mode, 2010.12.09 */
	...
	if (rtc_pdata->rtcalarm_time) 
	{
		...
		/*FIHTDC, MayLi adds for battery updates per 10 minutes in suspend mode, 2010.12.09 {+*/
