...
/* FIH, Karen Liao, 2011/01/27 { */
/* AI1D.B-9: [Audio] Set AI1 headset detection as active high. */
#include <mach/msm_smd.h>
static int productHWID = 0;
static int hsDetectLevel = 0;
/* } FIH, Karen Liao, 2011/01/27 */
...
*/	
	...
	if(gpio_get_value(data->gpio) == state)
	{
		...
		hook_switch_enable(state == hsDetectLevel, data->bHeadsetInserted);
		switch_set_state(&data->sdev, state);
		
		//change the sequence to delay switch_set_state for deciding the value of EnableAbnormalHS.
		// --- FIH, KarenLiao, @20091023: [F0X.FC-663]: Allow user to use the normal headset.
		
		data->bHeadsetInserted = (state == hsDetectLevel) ? true : false;		
...
static int gpio_switch_probe(struct platform_device *pdev)
{
	...
	switch_data->sdev.print_name = trout_h2w_print_name;
	...
	if((productHWID == Project_AI1D) || (productHWID == Project_AI1S))
	{
		printk(KERN_INFO "gpio_switch_probe() Project AI1D \n");
