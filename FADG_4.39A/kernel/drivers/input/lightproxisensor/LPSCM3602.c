...
static nv_stat_enum_type cmd_result = NV_DONE_S;

void get_proximity_threshold(void){	
	ncrpc_init();
  	oncrpc_task_start();

	cmd_result = nv_cmd_remote(NV_READ_F,  NV_PROXIMITY_THRESHOLD_I, &my_nv_item);
	
	if (NV_DONE_S != cmd_result){
		CM3623_PS_THD_VAL = 0x06;
		printk(KERN_ERR "%s: read ps NV fail(ret:%d)\n", __func__, client->name, cmd_result);
	}
	else{
		CM3623_PS_THD_VAL = my_nv_item.proximity_threshold;		
		printk(KERN_INFO "%s: read ps NV = %d\n", __func__, client->name, CM3623_PS_THD_VAL);
	}
}
*/
...
static irqreturn_t cm3623_isr( int irq, void * dev_id)
{
	...
	if(fih_proximity_level == 1) {
		printk("ps_wakelock\n");
		wake_lock_timeout(&ps_wakelock, HZ);
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	//set_proximity_threshold_to_NV(0x0F);	
	/* allocate memory */	
...
static int __init cm3623_init(void)
{
	...
	wake_lock_init(&ps_wakelock, WAKE_LOCK_SUSPEND, "ps_wakelock");
