...
#define i2ckybd_name "stmpe1601-keypad"
#define CMCS_HW_VER_EVT2 0
//#define DEBUG

static int __refdata qi2ckybd_probe(struct i2c_client *client,
...
// jones rename kpdev -> st1601_kpdev
// jones rename rd -> st1601_rd
struct input_dev *st1601_kpdev		= NULL;
struct i2ckybd_record *st1601_rd	= NULL;
static int HWID			= 0;
...
static uint16_t FIHKeypad_set[QKBD_IN_MXKYEVTS] =
{
	KEY_Q, 			KEY_W, 			KEY_E,      	KEY_R,     		KEY_T,   		KEY_Y,
	KEY_U, 			KEY_I, 			KEY_O,      	KEY_P,     		KEY_A,			KEY_S,  
	KEY_D, 			KEY_F, 			KEY_G,      	KEY_H,     		KEY_J,   		KEY_K,
	KEY_L, 			KEY_LEFTSHIFT, 	KEY_Z, 			KEY_X,     		KEY_C,   		KEY_V,
	KEY_B, 			KEY_N, 			KEY_M,  		KEY_COMMA, 		KEY_ENTER,		KEY_LEFTALT,	
	KEY_EMAIL,		KEY_SPACE, 		KEY_DOT,    	KEY_LANGUAGE, 		KEY_BACKSPACE,	KEY_RESERVED,
	KEY_RESERVED,	KEY_RESERVED,	KEY_RESERVED,	KEY_RESERVED,	KEY_RESERVED,   KEY_RESERVED,
	KEY_RESERVED,	KEY_RESERVED,	KEY_RESERVED,	KEY_RESERVED,	KEY_RESERVED,	KEY_RESERVED
};

static uint16_t AI1Keypad_set[QKBD_IN_MXKYEVTS] =
{
	KEY_LEFTALT, 		KEY_0, 			KEY_SPACE,  	KEY_SPACE,     		KEY_MENU,   		KEY_RIGHTALT,
	KEY_LEFTSHIFT, 		KEY_Z, 			KEY_X,      	KEY_C,     		KEY_V,			KEY_B,  
	KEY_A, 			KEY_S, 			KEY_D,      	KEY_F,     		KEY_G,   		KEY_H,
	KEY_Q, 			KEY_W, 			KEY_E,      	KEY_R,     		KEY_T,   		KEY_Y,
	KEY_U,			KEY_I, 			KEY_O,    	KEY_P, 			KEY_J,			KEY_K,
	KEY_L,			KEY_BACKSPACE,		KEY_N,		KEY_M,			KEY_DOT,  		KEY_ENTER,
	KEY_RESERVED,	KEY_RESERVED,	KEY_RESERVED,	KEY_RESERVED,	KEY_RESERVED,	KEY_RESERVED
};
/*
 * For END_KEY
 */
//2010.10.14 Vince rename msm_keypad_get_input_dev-> st_msm_keypad_get_input_dev
struct input_dev *st_msm_keypad_get_input_dev(void)
{	
	return st1601_kpdev;
}
EXPORT_SYMBOL(st_msm_keypad_get_input_dev);
...
//	bool bHeadsetInserted = false; // jones
	
	printk( "qi2ckybd_irqhandler+++++++++++\r\n" );
...
static int qi2ckybd_irqsetup(struct i2ckybd_record *kbdrec)
{
	...
	int rc = request_irq(KBDIRQNO(kbdrec), &qi2ckybd_irqhandler,
			     ...
			     i2ckybd_name, kbdrec);
	printk( "qi2ckybd_irqsetup+++++\r\n" );
	printk( "kbdrec->irqpin:%d\r\n", kbdrec->irqpin );
...
static int qi2ckybd_enablekybd(struct i2c_client *kbd)
{
	...
	printk( "[jjj]Enable keyboard function of STMPE1601\n");
	// Do S/W Reset
	rdat = 0x80;
	rc = kybd_write(kbd, STMPE1601_SYSTEM_CONTROL_REG, &rdat, sizeof(rdat));
	
	if (rc < 0) {
		printk( "FAILED: Write CTRL register (SOFT_RESET)(rc=%d)\n", rc);
		return rc;
	}
	...
	//set to 7 ms of debounce time and start keypad scanning	
	//vince 2010/12/03++
	//rdat = 0xFF;
	rdat = 0x0F;
	...
	rc = kybd_write(kbd, STMPE1601_SYSTEM_CONTROL_REG, &rdat, sizeof(rdat));
	if (rc < 0) {
		printk( "FAILED: Write CTRL register (Enable PWM and KPC)(rc=%d)\n", rc);
		return rc;
	} else {
		rc = kybd_read(kbd, STMPE1601_SYSTEM_CONTROL_REG, &rdat, sizeof(rdat));
		
		if ( 0x0F != (rdat & 0x1F) ) { //SLEEP Mode turn off, enable 
			printk( "FAILED: Enable PWM and KPC Failed!!(rc=%d) (rdat=0x%02x)\n", rc, rdat);
			return rc;
		} else
			printk( "SUCCESS: Enable PWM and KPC Successfully!!(rc=%d) (rdat=0x%02x)\n", rc, rdat);	
	}

	//change to Keypad function	
	
	/*vince, 2010/12/07++
	For DMQ, set unuse GPIO as output.*/
	
	//GPIO_0 and GPIO_1 are set as High, GPIO_15 are set as Low
	//0x82: STMPE1601_GPIO_SET_MSB_RFG
	//0x83: STMPE1601_GPIO_SET_LSB_RFG
	//0x84: STMPE1601_GPIO_CLR_MSB_RFG
	//0x85: STMPE1601_GPIO_CLR_LSB_RFG
	//0x88: STMPE1601_GPIO_SET_DIR_MSB_RFG
	//0x89: STMPE1601_GPIO_SET_DIR_LSB_RFG
	rdat = 0x80 ;//GPIO15
	rc = kybd_write(kbd, 0x88, &rdat, sizeof(rdat));
	rc = kybd_write(kbd, 0x84, &rdat, sizeof(rdat));
	
	rdat = 0x03 ;//GPIO0, and GPIO1
	rc = kybd_write(kbd, 0x89, &rdat, sizeof(rdat));
	rc = kybd_write(kbd, 0x83, &rdat, sizeof(rdat));
	/*vince 2010/12/07--*/
	//rdat = 0x55;
	rdat = 0x15 ;
	...
	rdat = 0x50 ;
	//rdat = 0x55;//0x5A; // to set to KEYPAD and PWM
	...
	//Scan PW: 128x period of internal clock and HIB_WK disabled
	//vince 2010/12/03
	rdat = 0xC0;
	//rdat = 0xE0;
...
static u32 scancode_to_keycode(struct i2ckybd_record *kbdrec, s32 scancode, s32 *kevent)
{
	...
	*kevent	= (scancode & 0x80) ? QKBD_IN_KEYRELEASE : QKBD_IN_KEYPRESS;
	...
	row	= (scancode & 0x78) >> 3;
	//row	= (scancode & 0x78) >> 3; // JONES
	col	= (scancode & 0x07) - 2;	
	index	= row * 6 + col;
	printk( "scancode:%d, row:%d, col:%d, index:%d\n", scancode, row, col, index );
...
static void qi2ckybd_fetchkeys(struct work_struct *work)
{
	...
	if (0 > rc) {
		printk( "FAIL: Clear STMPE1601 Interrupt Failed\n");		
	}
	printk( "qi2ckybd_fetchkeys---------------\n");
}
#if 0
...
static int hook_switch_release_thread(void *arg)
{
	...
	while (1) {
		...
		wake_lock(&st1601_rd->hook_sw_suspend_wake_lock);
#ifdef DEBUG
		printk(KERN_INFO "%s: Got complete signal\n", __func__);
#endif

		if (HoldingKeys[2].keyDown == 1) {
			input_report_key(st1601_kpdev, KEY_HEADSETHOOK, QKBD_IN_KEYRELEASE);
			input_sync(st1601_kpdev);
		}
		wake_unlock(&st1601_rd->hook_sw_suspend_wake_lock);
	}
	return 0;
}
...
static int hook_switch_request_irq_thread (void* arg)
{
	...
	while (1) {
		...
		wake_lock(&st1601_rd->hook_sw_request_irq_wake_lock);
		
		retry = 0;
		do {
			if ((temp_gpio_val = gpio_get_value(st1601_rd->hook_switch_pin) == 1) && !st1601_rd->bHookSWIRQEnabled) {
				//GPIO 94 is raised and hook switch irq has not been reqeusted yet.
				rc = request_irq(MSM_GPIO_TO_INT(st1601_rd->hook_switch_pin), &qi2ckybd_irqhandler,
					     (IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING), 
					     i2ckybd_name, st1601_rd);
				if (rc < 0) {
					printk(KERN_ERR
						"Could not register for %s interrupt "
						"(rc = %d)\n", i2ckybd_name, rc);
					...
					printk(KERN_INFO 
						...
						, __func__, MSM_GPIO_TO_INT(st1601_rd->hook_switch_pin));
						
					enable_irq_wake(MSM_GPIO_TO_INT(st1601_rd->hook_switch_pin));
					st1601_rd->bHookSWIRQEnabled = true;
					
					wake_unlock(&st1601_rd->hook_sw_request_irq_wake_lock);	//Hook switch IRQ is enabled and free wake lock.
					wait_for_completion(&headset_unplugged_irq_rel_comp);	//Stop here to waiting for irq releasing signal.
					wake_lock(&st1601_rd->hook_sw_request_irq_wake_lock);		//Get wake lock for later procedures and then Hook switch IRQ will be disabled.
					...
					free_irq(MSM_GPIO_TO_INT(st1601_rd->hook_switch_pin), st1601_rd);
					
					disable_irq_wake(MSM_GPIO_TO_INT(st1601_rd->hook_switch_pin));
					
					st1601_rd->bHookSWIRQEnabled = false;
		...
		wake_unlock(&st1601_rd->hook_sw_request_irq_wake_lock);
	}
...
static struct input_dev *create_inputdev_instance(struct i2ckybd_record *kbdrec)
{
	...
	if (idev != NULL) {
		...
		if(fih_read_new_hwid_mech_from_orighwid())
	   	{
	   		projectHWID = fih_read_product_id_from_orighwid() ;      
            		device_PHASE_ID = fih_read_phase_id_from_orighwid();

	   	}
	   	if(projectHWID==Project_AI1D || projectHWID==Project_AI1S)
	   	{
	   		idev->keycode		= AI1Keypad_set ;
	   		for (kidx = 0; kidx < QKBD_IN_MXKYEVTS; kidx++)
				__set_bit(AI1Keypad_set[kidx], idev->keybit);
	   	}
	   	else if(projectHWID==Project_DMQ || projectHWID==Project_DQE || projectHWID==Project_DQD)
	   	{
	   		//add read HWID for Domino Series, ++
			if((projectHWID == Project_DMQ && device_PHASE_ID >= Phase_PR2) ||
			   projectHWID == Project_DQD)
			{   
	   			FIHKeypad_set[27] = KEY_DOT ;
				FIHKeypad_set[32] = KEY_COMMA ;
			}
	   		idev->keycode		= FIHKeypad_set ;
...
static void qi2ckybd_connect2inputsys(struct work_struct *work)
{
	...
	printk( "qi2ckybd_connect2inputsys\r\n" );
...
static ssize_t power_state_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	spin_lock(&st1601_rd->stmpe1601_lock);
	gControllerPowerState = power_state;
	spin_unlock(&st1601_rd->stmpe1601_lock);
...
/*
#if 0 // 2101.10.13 Jones++, we should not use this any more for this domino project
	unsigned leds_state;
...
//2101.10.13 
//Jones DEVICE_ATTR(btn_brightness, 0644, NULL, btn_brightness_store);
#ifdef DEBUG
DEVICE_ATTR(debug_info, 0644, debug_info_show, debug_info_store);
#endif
//2010.10.14 Vince,
//DEVICE_ATTR(blink, 0644, NULL, blink_store);
DEVICE_ATTR(power_state, 0664, power_state_show, power_state_store);
...
static int __refdata qi2ckybd_remove(struct i2c_client *kbd)
{
...
static int qi2ckybd_resume(struct i2c_client *kbd)
{
	...
	spin_lock(&kbdrec->stmpe1601_lock);
	/*
	if(gControllerPowerState == STMPE1601_HIBERNATE_MODE){
		rdat = 0x20;
		gControllerPowerState = STMPE1601_NORMAL_MODE;
	}
	else{
	...
	*/
	...
	rc = kybd_write(kbd, STMPE1601_ICR_LSB_REG, &rdat, sizeof(rdat));
	if ( rc >= 0 ) {	
		printk("============ READ STMPE1601_ICR_LSB_REG=0x%x\n========", rdat);		
	} else {
...
static struct i2c_driver i2ckbd_st1601 = {
	.driver = {
		.owner = THIS_MODULE,
		.name  = i2ckybd_name,
	},
				    ...
				    const struct i2c_device_id *id)
{
	...
	st1601_rd = kzalloc(sizeof(struct i2ckybd_record), GFP_KERNEL);
	if (!st1601_rd) {
		dev_err(&client->dev, "i2ckybd_record memory allocation failed!!\n");
		return rc;
	}

	client->driver		= &i2ckbd_st1601;
	i2c_set_clientdata(client, st1601_rd);
	st1601_rd->mykeyboard		= client;
	setup_data 		= client->dev.platform_data;
	st1601_rd->mclrpin		= setup_data->gpioreset;
	st1601_rd->irqpin		= setup_data->gpioirq;
//	st1601_rd->volup_pin		= setup_data->gpio_vol_up;
//	st1601_rd->voldn_pin		= setup_data->gpio_vol_dn;
//	st1601_rd->hall_sensor_pin	= setup_data->gpio_hall_sensor;
//	st1601_rd->ring_switch_pin	= setup_data->gpio_ring_switch;
//	st1601_rd->hook_switch_pin	= setup_data->gpio_hook_switch;
	st1601_rd->bHookSWIRQEnabled	= false;
	spin_lock_init(&st1601_rd->stmpe1601_lock);

#ifdef CONFIG_HAS_EARLYSUSPEND
//	st1601_rd->early_suspend.level		= EARLY_SUSPEND_LEVEL_DISABLE_QWERTY_KEYPAD; // jjj
	st1601_rd->early_suspend.suspend	= qi2ckybd_early_suspend;
	st1601_rd->early_suspend.resume	= qi2ckybd_late_resume;
	register_early_suspend(&st1601_rd->early_suspend);
#endif

	wake_lock_init(&st1601_rd->ringer_sw_suspend_wake_lock, WAKE_LOCK_SUSPEND, "ringer_sw_suspend_work");
	wake_lock_init(&st1601_rd->hook_sw_suspend_wake_lock, WAKE_LOCK_SUSPEND, "hook_sw_suspend_work");
	wake_lock_init(&st1601_rd->hook_sw_request_irq_wake_lock, WAKE_LOCK_SUSPEND, "hook_sw_request_irq_work");

	//Initialize GPIO

	gpio_tlmm_config(GPIO_CFG(90, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_ENABLE);//reset
	gpio_tlmm_config(GPIO_CFG(91, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA), GPIO_ENABLE);//interrupt pin

//	gpio_tlmm_config(GPIO_CFG(90, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), GPIO_ENABLE);//reset
	
//	gpio_direction_output(st1601_rd->mclrpin, 0);
//	msleep(10);
//	gpio_direction_output(st1601_rd->mclrpin, 1);
//	msleep(10);
//	gpio_direction_output(st1601_rd->mclrpin, 0);
//	rc = qi2ckybd_config_gpio(st1601_rd);
//	if (rc)
//		goto failexit1;


	//Initialize IRQ
	INIT_WORK(&st1601_rd->qkybd_irqwork, qi2ckybd_fetchkeys);
	/*
	INIT_WORK(&st1601_rd->qkybd_volctrl, qi2ckybd_volkyctrl);
	INIT_WORK(&st1601_rd->qkybd_hallsensor, qi2ckybd_hallsensor);
	INIT_WORK(&st1601_rd->qkybd_ringswitch, qi2ckybd_ringswitch);
	INIT_WORK(&st1601_rd->qkybd_hookswitch, qi2ckybd_hookswitch);
	*/
	rc = qi2ckybd_irqsetup(st1601_rd);
	if (rc)
		goto failexit2;
	...
	if (rc < 0) {
		printk( "%s: register ring switch device failed!! <%d>", __func__, rc);
...
printk( "[jjj] qi2ckybd_probe----------------------\r\n" ); 
 ...
 failexit2:
//	free_irq(KBDIRQNO(st1601_rd), st1601_rd);
//	free_irq(MSM_GPIO_TO_INT(st1601_rd->volup_pin), st1601_rd);
//	free_irq(MSM_GPIO_TO_INT(st1601_rd->voldn_pin), st1601_rd);
//	free_irq(MSM_GPIO_TO_INT(st1601_rd->hall_sensor_pin), st1601_rd);
//	free_irq(MSM_GPIO_TO_INT(st1601_rd->ring_switch_pin), st1601_rd);
// failexit1:
//	qi2ckybd_release_gpio(st1601_rd);
//// There is some problem with kfree(st1601_rd);
//// 	kfree(st1601_rd);
...
static int __init qi2ckybd_init(void)
{
	int rc = -ENODEV;
	int project_HWID=0 ;
	// return i2c_add_driver(&i2ckbd_st1601);
	if(fih_read_new_hwid_mech_from_orighwid())
        {
   	     project_HWID = fih_read_product_id_from_orighwid() ;      
        }
        if(project_HWID==Project_AI1D || project_HWID==Project_AI1S || project_HWID==Project_DMQ || project_HWID==Project_DQD) 
	{
	rc = i2c_add_driver(&i2ckbd_st1601);
	printk( "qi2ckybd_init2, rc:%d\n", rc );
	return rc;
	}
	else
	{
		printk("without QWERTY keypad stmpe1601, rc=%d\n", rc);
		return -ENODEV;
	}
