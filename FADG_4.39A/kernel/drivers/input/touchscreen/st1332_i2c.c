...
#include <linux/slab.h>  //Added for DMP 7019 porting by Stanley (2010.10.17)
#include <linux/gpio.h>  //Added for DMP 7019 porting by Stanley (2010.10.17)
#include <asm/irq.h>
 ...
 ************************************************/
u8 st1332_buffer[8];
//Added new key event for FAD PR2 (2011.03.01)++
u8 st1332_keyevent[4];
//Added new key event for FAD PR2 (2011.03.01)--
...
//		pdata->keys=0;
    ...
    if(msm_cap_touch_fw_version >= 500)
    {
        ...
        {
            input_report_key(input, st1332_keyevent[0], 1);  //Added new key event for FAD PR2 (2011.03.01)
        ...
        {
            input_report_key(input, st1332_keyevent[1], 1);  //Added new key event for FAD PR2 (2011.03.01)
        ...
        {
            input_report_key(input, st1332_keyevent[2], 1);  //Added new key event for FAD PR2 (2011.03.01)
        ...
        {
            input_report_key(input, st1332_keyevent[3], 1);  //Added new key event for FAD PR2 (2011.03.01)
        ...
        else if ((pdata->keys == 0) && (bST1332_Soft1CapKeyPressed || bST1332_Soft2CapKeyPressed || bST1332_HomeCapKeyPressed || bST1332_BackCapKeyPressed))
        {
            ...
            {
                input_report_key(input, st1332_keyevent[0], 0);  //Added new key event for FAD PR2 (2011.03.01)
            ...
            {
                input_report_key(input, st1332_keyevent[1], 0);  //Added new key event for FAD PR2 (2011.03.01)
            ...
            {
                input_report_key(input, st1332_keyevent[2], 0);  //Added new key event for FAD PR2 (2011.03.01)
            ...
            {
                input_report_key(input, st1332_keyevent[3], 0);  //Added new key event for FAD PR2 (2011.03.01)
...
static int input_open(struct input_dev * idev)
{
	...
	set_irq_handler(client->irq, handle_bad_irq);
...
//	gpio_free(pdata->rst_gpio);	
    ...
    //Added the HWID scheme to decide TP resolution (2011.01.20)--
    //Added new key event for FAD PR2 (2011.03.01)++
    //if((fih_read_hwid_from_smem() >= CMCS_FAD_PR2) && (fih_read_hwid_from_smem() <= CMCS_FAD_PR8))
    if((fih_read_product_id_from_orighwid() == Project_FAD) && (fih_read_phase_id_from_orighwid() != Phase_PR1))  //Added for MP device by Stanley (2011.03.28)
    {
        st1332_keyevent[0] = KEY_BACK;
        st1332_keyevent[1] = KEY_KBDILLUMDOWN;
        st1332_keyevent[2] = KEY_HOME;
        st1332_keyevent[3] = KEY_SEARCH;
        st1332_msg(INFO, "[ST1332]Set SK event for FAD PR2 or later.");
    }
    else
    {
        st1332_keyevent[0] = KEY_KBDILLUMDOWN;
        st1332_keyevent[1] = KEY_HOME;
        st1332_keyevent[2] = KEY_BACK;
        st1332_keyevent[3] = KEY_SEARCH;
        st1332_msg(INFO, "[ST1332]Set SK event for Non-FAD PR2 or later.");
    }
    //Added new key event for FAD PR2 (2011.03.01)--
