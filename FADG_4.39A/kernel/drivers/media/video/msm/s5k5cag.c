...
#define SLAVE_ADDRESS_Chicony 0x78>>1
#define SLAVE_ADDRESS_Abico 0x5A>>1
int s5k5cag_CAMERA_VENDOR=0;//0: Chicony, 1:Abico
...
#define enable_CCM_AFIT_work //jones

#ifdef enable_CCM_AFIT_work // workkk
static bool bSensorPowerDown;//workkk
static bool set_done; //workkk
#endif //workkk
...
#ifdef enable_CCM_AFIT_work // workkk
static void tq_function ( struct work_struct *work ) //( struct work_struck *work ) // workkk
{ 
    mutex_lock(&s5k5cag_mutex);
    //if( !bSensorPowerDown )
    if( s5k5cag_ctrl != NULL  )
    {
        printk( "s5k5cag init AFIT & CCM--->\r\n" );
        if(s5k5cag_CAMERA_VENDOR==0)
        {        
        s5k5cag_i2c_write_w_table( &s5k5cag_ccm[0],
            ARRAY_SIZE(s5k5cag_ccm) );
        s5k5cag_i2c_write_w_table( &s5k5cag_afit[0],
            ARRAY_SIZE(s5k5cag_afit) );
        }
        else
        {
	        s5k5cag_i2c_write_w_table( &s5k5cag_ccm_Abico[0],
	            ARRAY_SIZE(s5k5cag_ccm_Abico) );
	        s5k5cag_i2c_write_w_table( &s5k5cag_afit_Abico[0],
	            ARRAY_SIZE(s5k5cag_afit_Abico) );        	
        }
    ...
    {
        //printk( "s5k5cag bSensorPowerDown is true, skip this\r\n" );
        printk( "s5k5cag_ctrl is NULLm skil AFIT & CCM\r\n" );
    }
...
{
	int i, freq = 0 ;  //Debbie add, 2011/07/19
	int32_t  rc;
	uint16_t reg;
	...
	for(i=0; i< retry_count; ++i)
	{
		...
		{
			s5k5cag_power_down();
			//s5k5cag_ctrl->sensordata->pdata->camera_gpio_off();

			printk("s5k5cag model_id = 0x%x\n", reg);

			msleep( 100*(i+1) );
			printk( "s5k5cag retry delay:%d\r\n", 200*(i+1) );			

			//s5k5cag_ctrl->sensordata->pdata->camera_gpio_on();				        
		    s5k5cag_power_up();
		}
		//	mdelay(10); 
		printk( "s5k5cag FIH 17 v4\r\n" );
		...
		{
			rc = s5k5cag_i2c_write_w_table( &s5k5cag_init_tbl_Abico[0],
				ARRAY_SIZE(s5k5cag_init_tbl_Abico) );
		}
		msleep(50);
//		printk( "s5k5cag open_count:%d, boom_count:%d\r\n", open_count, boom_count );
//		open_count++;		
// For test only+++		
//		if( boom_count % 4 == 0 && boom_count != 0 )
//		{
//		    printk( "s5k5cag boom!!!!!!!!!!\r\n" );
...
// For test only---
		...
		{
			rc = s5k5cag_i2c_write_w_table( &s5k5cag_init_tbl1_Abico[0],
				ARRAY_SIZE(s5k5cag_init_tbl1_Abico) );				
		}
		if(rc) {
		    printk( "s5k5cag continue1\r\n" );
		    continue ;
		}
		msleep(50);
	...
	//	s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x0F12, 0x0005 );		
		...
		{
			rc = s5k5cag_i2c_write_w_table( &s5k5cag_init_tbl2_Abico[0],
				ARRAY_SIZE(s5k5cag_init_tbl2_Abico) );			
		}
		
		if(rc) {
		    printk( "s5k5cag continue2\r\n" );
		    continue ;	
		}
		msleep(10);
		printk( "s5k5cag init table<--\r\n" );
	...
	//	
		rc = s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x002C, 0x7000 );
		rc = s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x002E, 0x327A );
		rc = s5k5cag_i2c_read_w( s5k5cag_client->addr, 0X0F12, &reg );	
		printk( "s5k5cag  IO driving current D:0x%x\r\n", reg );

	    rc = s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x002C, 0x7000 );	    
		rc = s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x002E, 0x01F8 );
		rc = s5k5cag_i2c_read_w( s5k5cag_client->addr, 0X0F12, &reg );	
		...
		if(freq !=78) 
			continue ;
		rc = s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x002C, 0x7000 );
		rc = s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x002E, 0x01FA );
		rc = s5k5cag_i2c_read_w( s5k5cag_client->addr, 0X0F12, &reg );	
		...
		if(freq !=78) 
			continue ;
		rc = s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x002C, 0x7000 );
		rc = s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x002E, 0x01FE );
		rc = s5k5cag_i2c_read_w( s5k5cag_client->addr, 0X0F12, &reg );	
		...
		if(freq !=78) 
			continue ;
		rc = s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x002C, 0x7000 );
		rc = s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x002E, 0x0200 );
		rc = s5k5cag_i2c_read_w( s5k5cag_client->addr, 0X0F12, &reg );	
...
#ifdef enable_CCM_AFIT_work // workkk	
    INIT_DELAYED_WORK( &tq, tq_function );// workkk
#endif // workkk    
...
//vince--
	...
	rc = s5k5cag_set_registers(1) ; //[DMQ.B-1598], Vince
	if (rc < 0)	{
	    printk(KERN_DEBUG "s5k5cag s5k5cag_set_registers failed rc:%d\r\n", rc);	
//	    boom_count++; // just for record crash times
		goto init_fail1;
    }

#ifdef enable_CCM_AFIT_work // workkk    
    set_done = false; // workkk
#endif // workkk    
...
#ifdef enable_CCM_AFIT_work //workkk
	bSensorPowerDown = true; //workkk    
    printk( "s5k5cag_cancel all of the works\r\n" );
    cancel_delayed_work( &tq ); //workkk
#endif // workkk    
...
static int32_t s5k5cag_video_config(int mode, int res)
{
    ...
    // maybe only need once
#ifdef enable_CCM_AFIT_work // workkk
    if( !set_done )
    {
        printk( "s5k5cag schedule work 700ms\r\n" );
        set_done = true;
        schedule_delayed_work( &tq, msecs_to_jiffies( 700 ) );// workkk
    }
#endif
...
static long s5k5cag_set_sharpness(int mode, int sharpness)
{
	...
	switch (sharpness) {
		case CAMERA_SHARPNESS_ZERO: {			
			...
			s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x0F12, 0x0000 );//s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x0F12, 0xFF81 );
		...
		case CAMERA_SHARPNESS_POSITIVE_1: {			
			...
			s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x0028, 0x7000 );
			s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x002A, 0x0212 );
			s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x0F12, 0x003F );//s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x0F12, 0x0000 );    		
			if (rc < 0)
				return rc;
		}
		break;
		...
		case CAMERA_SHARPNESS_POSITIVE_2: {			
			...
			s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x0028, 0x7000 );
			s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x002A, 0x0212 );
			s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x0F12, 0x007F );//s5k5cag_i2c_write_w( s5k5cag_client->addr, 0x0F12, 0x007F );
			if (rc < 0)
				return rc;
		}
		break;
...
//vince--
	...
	// mdelay(20);
    mdelay(1);
	printk( " s5k5cag_sensor_probe s5k5cag_client->addr=0x%x\r\n",s5k5cag_client->addr );
	rc = s5k5cag_probe_init_sensor(info);
	if (rc < 0)
	{
		s5k5cag_client->addr=SLAVE_ADDRESS_Abico;
		printk( " s5k5cag_sensor_probe s5k5cag_client->addr=0x%x\r\n",s5k5cag_client->addr );
	rc = s5k5cag_probe_init_sensor(info);
	if (rc < 0)
		{
		...
		s5k5cag_CAMERA_VENDOR=1;
	}
	else
	{
		s5k5cag_CAMERA_VENDOR=0;
	}
	printk( " s5k5cag_sensor_probe s5k5cag_CAMERA_VENDOR=%d\r\n",s5k5cag_CAMERA_VENDOR );

	s->s_init = s5k5cag_sensor_open_init;
	s->s_release = s5k5cag_sensor_release;
	s->s_config  = s5k5cag_sensor_config;
	s->s_camera_type=BACK_CAMERA_2D;
	s->s_mount_angle=90;//front camera angle.
