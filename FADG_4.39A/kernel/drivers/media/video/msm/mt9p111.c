...
DEVICE_ATTR(mt9p111regset, 0664, NULL, mt9p111_I2C_setting_attr);
...
static long mt9p111_set_effect(int mode, int effect)
{
    ...
    if(mt9p111_effect_mode==effect)
    	printk("mt9p111_effect_mode==effect \n") ;
//        return 0;
...
static long mt9p111_set_autofocus(int mode, int autofocus)
{
	...
	//AF_postition_settings
		...
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB019, 0x18, BYTE_LEN);  // AF_FS_POS_1
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB01A, 0x30, BYTE_LEN);	// AF_FS_POS_2
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB01B, 0x48, BYTE_LEN);	// AF_FS_POS_3
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB01C, 0x60, BYTE_LEN);	// AF_FS_POS_4
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB01D, 0x80, BYTE_LEN);	// AF_FS_POS_5
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB01E, 0xA0, BYTE_LEN);	// AF_FS_POS_6
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB01F, 0xC0, BYTE_LEN);	// AF_FS_POS_7
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB020, 0xE0, BYTE_LEN);	// AF_FS_POS_8
	...
	//AF_postition_settings
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB018, 0x00, BYTE_LEN);	// AF_FS_POS_0
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB019, 0x10, BYTE_LEN);	// AF_FS_POS_1
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB01A, 0x20, BYTE_LEN);	// AF_FS_POS_2
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB01B, 0x38, BYTE_LEN);	// AF_FS_POS_3
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB01C, 0x40, BYTE_LEN);	// AF_FS_POS_4
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB01D, 0x50, BYTE_LEN);	// AF_FS_POS_5
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB01E, 0x60, BYTE_LEN); 	// AF_FS_POS_6
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB01F, 0x70, BYTE_LEN); 	// AF_FS_POS_7
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB020, 0x80, BYTE_LEN);	// AF_FS_POS_8
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB021, 0x90, BYTE_LEN);	// AF_FS_POS_9
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB022, 0xA0, BYTE_LEN);	// AF_FS_POS_10
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB023, 0xB0, BYTE_LEN);	// AF_FS_POS_11
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB024, 0xC0, BYTE_LEN);	// AF_FS_POS_12
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB025, 0xD0, BYTE_LEN);	// AF_FS_POS_13
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB026, 0xE0, BYTE_LEN);	// AF_FS_POS_14
		(void)mt9p111_i2c_write(mt9p111_client->addr, 0xB027, 0xF0, BYTE_LEN);	// AF_FS_POS_15
...
//Div6-PT2-MM-CH-AF-00+]
struct work_struct flashledoff_work;
static void  mt9p111_flashled_off_work(struct work_struct *work);
static long mt9p111_set_sensor_mode(int mode)
{
	...
	case SENSOR_SNAPSHOT_MODE:
		...
		{	
			...
			init_completion(&mt9p111_flashled_comp);
			schedule_work(&flashledoff_work);
...
//test DVDD--
	...
	//mdelay(10);
	    ...
	    rc2 = vreg_set_level(vreg_camera, 2650);
	    printk(KERN_ERR "%s: Set AVDD to 2.65V for DMP!\n", __func__);
	    if (rc2) {
...
*/	
	INIT_WORK(&flashledoff_work, mt9p111_flashled_off_work);
