...
// add for DMP PR2+++
	...
	if(g_HM0357_PRODUCT_ID==Project_DMP || g_HM0357_PRODUCT_ID==Project_DP2)
	{	
...
static int hm0357_reg_init(void)
{
	...
	if (rc < 0)
		return rc;
	if(Project_DMP==g_HM0357_PRODUCT_ID || g_HM0357_PRODUCT_ID==Project_DP2)
	{
...
//HW on DMO reverses camera module 180 degrees comparing to DMP.++
		printk("hm0357_set_brightness: We utilize this funtion to set sensor output no-flipping.!!");	
		if(Project_DMO==g_HM0357_PRODUCT_ID || Project_DMT==g_HM0357_PRODUCT_ID )
		{
			printk("Project_DMO;Due to camera module reversed, set 0x0006->0x3.");
			rc = hm0357_i2c_write(HM0357_I2C_WRITE_SLAVE_ID,
					0x0006, 0x3,
					BYTE_LEN);
		}
...
//[DMP.B-231]Henry added this for reversing front camera output data. This makes raw data no mirror.++
			rc = hm0357_i2c_write(HM0357_I2C_WRITE_SLAVE_ID,
				0x0006, 0,
				BYTE_LEN);
			printk("Other projects, set 0x0006 to 0!!");	
//[DMP.B-231]Henry added this for reversing front camera output data. This makes raw data no mirror.--
		}
...
*/
	...
	rc = vreg_set_level(vreg_camera, 2650);
	printk(KERN_ERR "%s: Set AVDD to 2.65V for DMP.\n", __func__);
	if (rc) {
		printk(KERN_ERR "%s: vreg set level to 2.8V failed (%d)\n", __func__, rc);
		goto init_fail;
...
/* FIH, Henry Juang, 2010/11/01 } */
	...
	s->s_mount_angle=270;//front camera angle.
