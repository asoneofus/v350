		...
		int timeout)
{
	...
	if (list_empty_careful(&queue->list)) {
		if (!rc) {
			pr_err("!rc=%d,queue->wait=%d,queue->list=%d! \n",rc,(unsigned int)&queue->wait,(unsigned int)&queue->list);//Henry added for monitoring [DP2.B-511]DUT pops up force close when use camera.++
			rc = -ETIMEDOUT;
			pr_err("%s: wait_event error %d\n", __func__, rc);			
			return ERR_PTR(rc);
		} else if (rc < 0) {
			pr_err("rc < 0=%d,queue->wait=%d,queue->list=%d! \n",rc,(unsigned int)&queue->wait,(unsigned int)&queue->list);//Henry added for monitoring [DP2.B-511]DUT pops up force close when use camera.++
			pr_err("%s: wait_event error %d\n", __func__, rc);
