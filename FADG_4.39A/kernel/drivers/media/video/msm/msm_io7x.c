...
int msm_camio_sensor_clk_on(struct platform_device *pdev)
{
	...
	if (!mdcbase) {
		rc = -EINVAL;
		printk("****** rc=%d mdcbase=0x%x, camio_ext.mdcphy=0x%x,camio_ext.camifpadsz=0x%x \n",rc,(unsigned int)mdcbase,(unsigned int)camio_ext.mdcphy,(unsigned int)camio_ext.mdcsz);
		goto mdc_no_mem;
	}
	printk("mdcbase=0x%x, camio_ext.mdcphy=0x%x,camio_ext.camifpadsz=0x%x \n",(unsigned int)mdcbase,(unsigned int)camio_ext.mdcphy,(unsigned int)camio_ext.mdcsz);
