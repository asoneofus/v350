...
//FIHTDC, add for 7019 battery charging, MayLi, 2010.10.28 {+
#ifdef CONFIG_FIH_FXX
int AC_flag=0; /* FIHTDC, Add for partial charge when over heat, MayLi, 2011.07.14 */
...
static inline enum chg_type usb_get_chg_type(struct usb_info *ui)
{
	if ((readl(USB_PORTSC) & PORTSC_LS) == PORTSC_LS)
	{
		...
		#ifdef CONFIG_FIH_FXX
		AC_flag = 1;  /* FIHTDC, Add for partial charge when over heat, MayLi, 2011.07.14 */
	...
	else
	{
		...
		// MayLi adds for set charging current, 2010.10.28 {+
		#ifdef CONFIG_FIH_FXX
		AC_flag = 0;  /* FIHTDC, Add for partial charge when over heat, MayLi, 2011.07.14 */
...
//WilsonWHLee 2010/02/22 5110 porting ++++++++++++++++
	...
	if (retval != 0)
		printk("failed to create sysfs entry: (dev_attr_switch_enable) error: (%d)\n",
					retval);

	retval = device_create_file(&ui->gadget.dev, &dev_attr_hs_connect);
	if (retval != 0)
		printk("failed to create sysfs entry: (dev_attr_hs_connect) error: (%d)\n",
					retval);
//WilsonWHLee 2010/02/22 5110 porting -----------------
/* FIH, WilsonWHLee, 2010/10/18 { */
/* [FXX_CR], Virtual file for disabling USB controller */
	...
	if (retval != 0)
		printk("failed to create sysfs entry: (dev_attr_turnoffUSB) error: (%d)\n",
					retval);
/* } FIH, WilsonWHLee, 2010/10/18  */	
...
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	...
	device_remove_file(&dev->gadget.dev, &dev_attr_turnoffUSB);
