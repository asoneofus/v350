...
static int fih_bkl_is_5led=TRUE;
...
//	unsigned long flags;
	...
	while(!checkPanelON)
	{
		...
		if((fih_read_product_id_from_orighwid() == Project_DQE || fih_read_product_id_from_orighwid() == Project_DQD) && checkTimes>0) 
			msleep(50);
		//FXX_CR, Neo Chen, 2009.12.31 Let backlight not to open until LCM is on
		if(checkTimes>=100) break;
	}
...
static int rt9365_probe(struct platform_device *pdev)
{
	int rc=0;
...
/* FIH, IvanCHTang, 2011/4/14 { */
/* [FXX_CR], register shutdown function to be notified by platform */
static void rt9365_shutdown(struct platform_device *pdev)
{
	printk( KERN_INFO "Driver shutdown: %s\n", __func__ );
	// Set 0 degree of backlight
	rt9365_set_level(0);
}
/* } FIH, IvanCHTang, 2011/4/14 */
...
/* When bkl is rt9365 or sc668, return true */
...
static int __init rt9365_init(void)
{
	...
	if(!(Project_DPD <=fih_read_product_id_from_orighwid() && fih_read_product_id_from_orighwid() <=Project_FAD)){
		...
		fih_bkl_is_5led=FALSE;
		fih_lcm_is_power_by_sc668=FALSE;
		if(fih_read_product_id_from_orighwid()==Project_DMQ && fih_read_phase_id_from_orighwid() <= Phase_PR1)
		{
	...
	printk( KERN_INFO "%s(): fih_bkl_is_sc668(%d), fih_lcm_is_power_by_sc668(%d), fih_bkl_is_5led(%d)\n", 
		__func__, fih_bkl_is_sc668, fih_lcm_is_power_by_sc668,fih_bkl_is_5led);	
