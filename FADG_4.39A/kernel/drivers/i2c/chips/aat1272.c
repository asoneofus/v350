...
#endif
/* } FIH, Charles Huang, 2009/12/28 */
static unsigned mpp_19= 18;//Pull MPP_19(FLEN) high to start Voltage out.
static unsigned mpp_1= 0;//Pull MPP_1(EN) high to enable I2C.
// add for HW_PR2, jones +++
int g_PRODUCT_ID = 0;
int g_PHASE_ID = 0;
// add for HW_PR2, jones ---
...
/******************Driver Functions*******************/
// add for HW_PR2, jones +++
static void flash_drv_en( int enable )
{	
	int NOT_PAHSE_PR2 = 1;
	printk( "aat1272 flash_drv_en:%d\r\n", enable );
	
	if(g_PRODUCT_ID==Project_DMP|| g_PRODUCT_ID==Project_DPD|| g_PRODUCT_ID==Project_DP2)
	{		
		printk( "aat1272 PRODUCE_ID : 0x4 || 0x5 || 0x6\r\n" );
		if( g_PHASE_ID >= Phase_PR2 )
		{	
			printk( "aat1272 Phase >= PR2\r\n" );
			NOT_PAHSE_PR2 = 0;
			gpio_tlmm_config(GPIO_CFG(37, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), GPIO_ENABLE);
	...
	if( g_PRODUCT_ID == Project_AI1D || g_PRODUCT_ID == Project_AI1S)
    {
        NOT_PAHSE_PR2 = 0;
        gpio_tlmm_config(GPIO_CFG(76, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), GPIO_ENABLE);			
		if( enable )
		{
	...
	if( NOT_PAHSE_PR2 )
	{
		printk( "aat1272 NOT_PAHSE_PR2\r\n" );
		if( enable ) 
		{
			mpp_config_digital_out(mpp_1,MPP_CFG(MPP_DLOGIC_LVL_MSMP,
...
void brightness_onoff(int on)
{
	...
	//static struct mpp *mpp_1;

	//mpp_1 = mpp_get(NULL, "mpp22");
	aat1272_user_g0(KERN_INFO "flashdriver_store::on: %d\n", on);
	if (on==1){
		//ret=mpp_config_digital_out(mpp_1,MPP_CFG(MPP_DLOGIC_LVL_MSMP,
		//	MPP_DLOGIC_OUT_CTRL_HIGH));
		flash_drv_en( 1 );
...
static ssize_t brightness_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk(KERN_WARNING "aat1272  brightness_enable: %d.\n",enable);
...
DEVICE_ATTR(flashenable, 0664, NULL, flash_enable);
DEVICE_ATTR(brightness, 0664, NULL, brightness_enable);
DEVICE_ATTR(flashreg, 0664, flash_read, flash_write);
...
// add for HW_PR2, jones +++
	if(fih_read_new_hwid_mech_from_orighwid())
   	{
   		g_PRODUCT_ID = fih_read_product_id_from_orighwid() ;      
   		g_PHASE_ID = fih_read_phase_id_from_orighwid();
   	}
// add for HW_PR2, jones ---		
	return ret;
}
