...
#define KEYPAD_BL_LEVEL_10mA	255
	...
	enum led_brightness value)
{
	...
	{
		if(ProductID >= Project_AI1S && ProductID <= Project_AI1D)
			ret = pmic_set_led_intensity(LED_KEYPAD, value / KEYPAD_BL_LEVEL_10mA);
		else
...
static int __init btn_pmic_led_init(void)
{
	...
	if(ProductID >= Project_DPD && !(ProductID >= Project_AI1S && ProductID <= Project_AI1D))
	{
