...
//ada1001007
DEVICE_ATTR(kpd_blink, 0664, NULL, kpd_blink_store);
DEVICE_ATTR(blink_onMS, 0664, NULL, blink_onMS_store);
DEVICE_ATTR(blink_offMS, 0664, NULL, blink_offMS_store);
//ada1001007
...
static int __init sc668_led_init(void)
{
	...
	if(!(Product_ID >= Project_DPD && Phase_ID <= Phase_PR1) || (Product_ID >= Project_DMO)) //Some Dominol+ series PR1 not to set CHG_LED_CTRL
	{
		...
		gpio_tlmm_config(GPIO_CFG(CHG_LED_CTRL, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		gpio_set_value(CHG_LED_CTRL, 1);
	}
