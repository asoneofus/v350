...
/* FIH, Debbie, 2010/11/26 { */
/* remove serial dynamically */
...
/* [workaround]add UART2DM for UART */
			  ...
			  unsigned int oldstate)
{
	...
	/* Debbie, 2011/07/17 { */
	/* porting 7220 */
	port->uartclk = clk_get_rate(msm_hsl_port->clk);
	/* Debbie, 2011/07/17 } */
}
...
static int msm_serial_hsl_init(void)
{
	...
	/* remove serial dynamically */
	//#ifdef CONFIG_FIH_REMOVE_SERIAL_DYNAMICALLY
	struct clk *uart2dm_clk;printk(KERN_INFO "msm_serial_hsl:kernel init\n");
	if(/*!fih_read_uart_switch_from_smem()*/0) {
		printk(KERN_INFO "msm_serial: Disable UART clock\n");
		gpio_tlmm_config(GPIO_CFG(21, 5, GPIO_INPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_DISABLE);
		gpio_tlmm_config(GPIO_CFG(108, 2, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_DISABLE);
		uart2dm_clk = clk_get(&msm_device_uart_dm12.dev, "uartdm_clk");        
		if(!IS_ERR(uart2dm_clk)){            
			clk_enable(uart2dm_clk);   //If clk_enable is not called, calling clk_disable will enter ram dump.            
			clk_disable(uart2dm_clk);        
		}
		return 0; 
	}
	//#endif
