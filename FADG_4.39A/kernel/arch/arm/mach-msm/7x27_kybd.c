/* FIH, JiaHao, 2010/10/07 { */
/* special for kzalloc and kfree of 7019 */
...
#include <linux/completion.h>//vince
...
//FIH, VinceWLLiu, enable or disable key GPIO configuration, 2011/1/31s+++
static bool b_volUp_GPIO = false ;
static bool b_volDn_GPIO = false ;
static bool b_key1_GPIO = false ;
static bool b_key2_GPIO = false ;
static bool b_camT_GPIO = false ;
static bool b_camF_GPIO = false ;
static bool b_hook_GPIO = false ;
static bool b_center_GPIO = false ;
//FIH, VinceWLLiu, enable or disable key GPIO configurations, 2011/1/31---
...
//FIH vince 2010/11/10++
int g_PRODUCTHWID=0 ;
//FIH vince 2010/11/10--
static int EnableKeyInt = 1; // 1:enable key interrupt
//-FIH_Misty
#endif
...
//
static bool g_key1_status ;
static bool g_key2_status ;
static bool g_camT_status ;
static bool g_camF_status ;
static bool g_volup_status ;
static bool g_voldn_status ;
static bool g_hook_status ;
//
...
/* } FIH, NicoleWeng, @20100506  */	
    ...
    if (kbdrec->kybd_connected) {
		...
		if(b_key2_GPIO)
		{
			if (MSM_GPIO_TO_INT(kbdrec->key_2_pin) == irq){
		...
		if(b_volUp_GPIO)
		{
   		...
   		if(b_volDn_GPIO)
   		{
        		if (MSM_GPIO_TO_INT(kbdrec->voldn_pin) == irq) {
		...
		if(b_camT_GPIO)
		{
			if (MSM_GPIO_TO_INT(kbdrec->cam_sw_t_pin) == irq){
				schedule_work(&kbdrec->kybd_camkey1);
     			} 
		}
		if(b_camF_GPIO)
		{
			if (MSM_GPIO_TO_INT(kbdrec->cam_sw_f_pin) == irq) {
		...
		if(b_hook_GPIO)
		{
			if (MSM_GPIO_TO_INT(kbdrec->hook_sw_pin) == irq){
...
/* add center key                 */
		...
		if(b_center_GPIO)
		{
			if (MSM_GPIO_TO_INT(kbdrec->center_pin) == irq){
...
/* clear pending interrupt        */
	/* FIH, JiaHao, 2010/09/10 { */
	/* modify for 7015 */
	/* 7015 has no this func, marked for build pass */
...
/* } FIH, PeterKCTseng, @20090603 */
    	...
    	g_hook_status = gpio_get_value(rd->hook_sw_pin);
	return rc;
...
/* The active type of input pin   */
 ...
 //   int HWID = FIH_READ_HWID_FROM_SMEM(); 
	...
	key_1_val		= (bool)gpio_get_value(kbdrec->key_1_pin);
	if(g_camF_status == key_1_val)
	{
		printk(" Q7x27_kybd_generalkey1 filter keyevent\n");
		return;
	}
	g_key1_status = key_1_val;
...
/* } FIH, PeterKCTseng, @20090520 */
	...
	//printk(KERN_INFO "KEY 2 <%d>\n", key_2_val);
	msleep(DEBOUNCE_TIME);
	key_2_val		= (bool)gpio_get_value(kbdrec->key_2_pin);
	if(g_key2_status == key_2_val)
	{
		printk(" Q7x27_kybd_generalkey2 filter keyevent\n");
		return;
	}
	g_key2_status = key_2_val;
	disable_irq(MSM_GPIO_TO_INT(kbdrec->key_2_pin));
//++++++++++++++++++++++++++++++FIH_F0X_misty
...
/* } FIH, PeterKCTseng, @20090520 */
	...
	if(g_volup_status  == volup_val)
	{
		printk(" Q7x27_kybd_volkey1 filter keyevent\n");
		return;
	}
	g_volup_status  = volup_val;
...
//+++++++++++++++++++++++++++++++FIH_F0X_misty
    if(EnableKeyInt)
    {
        ...
        // FIH, WillChen, 2009/08/14 --
            	...
            	input_sync(idev);
            	printk(KERN_INFO "FIH: input_sync KEY_VOLUMEUP\n");	
            }        	
...
/* } FIH, PeterKCTseng, @20090520 */
	...
	if(g_voldn_status  == voldn_val)
	{
		printk(" Q7x27_kybd_volkey2 filter keyevent\n");
		return;
	}
	g_voldn_status  = voldn_val;
...
/* } FIH, PeterKCTseng, @20090520 */
	...
	msleep(DEBOUNCE_TIME);
	cam_sw_t_val		= (bool)gpio_get_value(kbdrec->cam_sw_t_pin);
	if(g_camT_status == cam_sw_t_val)
	{
		printk(" Q7x27_kybd_camkey1 filter keyevent\n");
		return;
	}
	g_camT_status = cam_sw_t_val;
...
/* } FIH, PeterKCTseng, @20090520 */
	msleep(DEBOUNCE_TIME);
	cam_sw_f_val		= (bool)gpio_get_value(kbdrec->cam_sw_f_pin);
	if(g_camF_status == cam_sw_f_val)
	{
		printk(" Q7x27_kybd_camkey2 filter keyevent\n");
		return;
	}
	g_camF_status = cam_sw_f_val;
...
//		msleep(500);
	...
	msleep(DEBOUNCE_TIME);
	hook_sw_val 		= (bool)gpio_get_value(kbdrec->hook_sw_pin);
	if(g_hook_status  == hook_sw_val)
	{
		printk(" Q7x27_hook_switchkey filter keyevent\n");
		return;
	}
	g_hook_status = hook_sw_val;
...
//FIH, Vince 2010/11/10+++
   ...
   {
   	g_PRODUCTHWID = fih_read_product_id_from_orighwid() ;      
   }
   else
   {
   	printk("using old hwid mechanism...\n") ;
...
// FIH, vince 2010/11/10+++
     ...
     printk("FIH: Project Detected Product HWID:%d\n",g_PRODUCTHWID) ;
    if (g_PRODUCTHWID==Project_DMP|| g_PRODUCTHWID==Project_DPD|| g_PRODUCTHWID==Project_DP2) 
    {
	printk("FIH: Product HWID:%d\n",g_PRODUCTHWID) ;
    ...
    else if (g_PRODUCTHWID  == Project_DMQ || g_PRODUCTHWID  == Project_DMT || g_PRODUCTHWID  == Project_DMO || g_PRODUCTHWID  == Project_DQD) {
	printk("FIH: Product HWID:%d\n ",g_PRODUCTHWID);
    ...
    else if (g_PRODUCTHWID==Project_FAD) 
    {
	printk("FIH: Product HWID:%d\n",g_PRODUCTHWID) ;
        rd->active.volup_pin_actype     = ACTIVE_LOW;
        rd->active.voldn_pin_actype     = ACTIVE_LOW;
    ...
    else if(g_PRODUCTHWID==Project_AI1D || g_PRODUCTHWID==Project_AI1S)
    {
    	setup_data->key_1_pin = 88;
	setup_data->cam_sw_t_pin = 17;
        //setup_data->cam_sw_f_pin = 88;
	//setup_data->hook_sw_pin;
        //g_End = false ;
    	rd->active.volup_pin_actype     = ACTIVE_LOW;
        rd->active.voldn_pin_actype     = ACTIVE_LOW;
    ...
    else 
    {
		...
		printk(KERN_ERR "target product can not be recognized!! \n");
    }
    
// FIH, vince 2010/11/10---
#if 0 //old read hwid mechanism
...
//get key's gpio status
    if(b_key1_GPIO)
    	g_key1_status = gpio_get_value(rd->key_1_pin);
    if(b_key2_GPIO)
    	g_key2_status = gpio_get_value(rd->key_2_pin);
    if(b_camT_GPIO)
    	g_camT_status = gpio_get_value(rd->cam_sw_t_pin);
    if(b_camF_GPIO)
    	g_camF_status = gpio_get_value(rd->cam_sw_f_pin);
    if(b_volUp_GPIO)
    	g_volup_status = gpio_get_value(rd->volup_pin);
    if(b_volDn_GPIO)
    	g_voldn_status = gpio_get_value(rd->voldn_pin);

    printk("7x27_kybd probe done!!!\n") ;
    return 0;
#if 0//misty+++
 failexit2:
