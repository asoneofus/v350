...
void fih_smem_alloc(void)
{
    ...
    /* FIH, JiaHao, 2011/05/01 { */
    /* get orig_hwid from smem_hwid*/
    fih_orig_hwid = fih_smem_info->hw_id;
    /* FIH, JiaHao, 2011/03/10 { */
    /* change OrigHWID from DMQ be DQD */
    if (fih_read_product_id_from_orighwid() == Project_DMQ) {
        fih_orig_hwid = ((fih_orig_hwid & 0xFFFFFF00) | Project_DQD);
    }
    /* FIH, JiaHao, 2011/03/10 } */
    /* map orig_hwid to hwid */
    ...
    /* FIH, JiaHao, 2010/08/04 { */
    #if (1)
    printk(KERN_INFO "fih_smem_alloc() : fih_orig_hwid = 0x%08X\n",  fih_orig_hwid);
    printk(KERN_INFO "fih_smem_alloc() : fih_hwid = 0x%08X\n",  fih_hwid);
    ...
    /* FIH, JiaHao, 2010/12/17 { */
    /* New HWID Mechanism */
    printk(KERN_INFO "fih_smem_alloc() : HAC_Support = %d\n", fih_read_hac_support_from_orighwid());
    printk(KERN_INFO "fih_smem_alloc() : Headdset_Type = %d\n", fih_read_headset_type_from_orighwid());
    printk(KERN_INFO "fih_smem_alloc() : SIM_ID = %d\n", fih_read_sim_id_from_orighwid());
    printk(KERN_INFO "fih_smem_alloc() : Band_ID = %d\n", fih_read_band_id_from_orighwid());
    printk(KERN_INFO "fih_smem_alloc() : Phase_ID = %d\n", fih_read_phase_id_from_orighwid());
    printk(KERN_INFO "fih_smem_alloc() : Product_ID = %d\n", fih_read_product_id_from_orighwid());
    /* FIH, JiaHao, 2010/12/17 } */
    #endif
    /* FIH, JiaHao, 2010/08/04 } */
}
...
unsigned int fih_read_product_id_from_orighwid(void)
{
	...
	if (fih_read_new_hwid_mech_from_orighwid()) {
		RetVal = (fih_orig_hwid&0x000000FF);
	}
	return RetVal;
}
...
unsigned int fih_read_phase_id_from_orighwid(void)
{
	unsigned int RetVal = (0);
	if (fih_read_new_hwid_mech_from_orighwid()) {
		RetVal = ((fih_orig_hwid>>8)&0x000000FF);
	}
	return RetVal;
}
...
unsigned int fih_read_band_id_from_orighwid(void)
{
	unsigned int RetVal = (0);
	if (fih_read_new_hwid_mech_from_orighwid()) {
		RetVal = ((fih_orig_hwid>>16)&0x000000FF);
	}
	return RetVal;
}

unsigned int fih_read_sim_id_from_orighwid(void)
{
	unsigned int RetVal = (0);
	if (fih_read_new_hwid_mech_from_orighwid()) {
		RetVal = ((fih_orig_hwid>>24)&0x00000003);
	}
	return RetVal;
}
...
unsigned int fih_read_headset_type_from_orighwid(void)
{
	unsigned int RetVal = (0);
	if (fih_read_new_hwid_mech_from_orighwid()) {
		RetVal = ((fih_orig_hwid>>28)&0x00000001);
	}
	return RetVal;
}
...
unsigned int fih_read_hac_support_from_orighwid(void)
{
	unsigned int RetVal = (0);
	if (fih_read_new_hwid_mech_from_orighwid()) {
		RetVal = ((fih_orig_hwid>>29)&0x00000001);
	}
	return RetVal;
}
/* FIH, JiaHao, 2010/12/17 } */
