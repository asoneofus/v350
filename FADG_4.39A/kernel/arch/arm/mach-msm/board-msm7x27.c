...
#include <linux/st1332_i2c.h>  //Added for capacitive touch panel, StanleyCheng, [DMP_porting]
#endif
/* FIH, Stanley } */
...
//Wilson added ++
	...
	{
		.product_id	= 0xc002,
		.num_functions	= ARRAY_SIZE(usb_functions_recovery),
		.functions	= usb_functions_recovery,
	},
	{
		.product_id	= 0xc004,
...
static struct usb_mass_storage_platform_data mass_storage_pdata = {
	.nluns		= 2, // lun0: for sdcard, lun1: for virtual cdrom
...
static char serialnumber[64] = "123456789ABCDEF";
static struct android_usb_platform_data android_usb_pdata = {
	...
	.serial_number = serialnumber,//"1234567890ABCDEF",
...
static int __init board_serialno_setup(char *serialno)
{
	...
	//android_usb_pdata.serial_number = serialno; //WisonWHLee removed
	return 1;
}
...
static struct snd_endpoint snd_endpoints_list[] = {
	...
	/* FIH, KarenLiao, 2010/11/04 { */
	/* Add new devices for FM function. */	
	SND(FM_HEADSET, 28),
	SND(FM_SPEAKER, 29),
	SND(SPEAKER_VOICE_RECOGNITION, 30),    //Terry 2011.5.19
	SND(SPEAKER_VOICE_COMMUNICATION, 31),	 //Terry 2011.5.19
	SND(CURRENT, 33),
...
static struct platform_device rt9365_device = {
	.name = "rt9365",
};
/* ChandlerKang, [porting_rt9365] }*/

/* ChandlerKang, [mq4_porting] {*/
static struct platform_device tps61160_device = {
	.name = "tps61160",};
/* ChandlerKang, [mq4_porting] }*/
...
/* FIH, JamesKCTung, 2009/06/30 { */
#ifdef CONFIG_FIH_FXX
    rc = gpio_request(35, "WLAN_EN");
    if (rc)	printk(KERN_ERR "%s: WLAN_EN 35 setting failed! rc = %d\n", __func__, rc);
    rc = gpio_request(27, "BT_EN");
    if (rc)	printk(KERN_ERR "%s: BT_EN 27 setting failed! rc = %d\n", __func__, rc);
#endif
/* } FIH, JamesKCTung, 2009/06/30 */
...
/* FIH, Neo Chen, 2010/10/28, Domino button LEDs{ */
static struct platform_device msm_device_pmic_btn_leds = {
	.name   = "button-leds",
	.id = -1,
};
/* } FIH, Neo Chen, 2010/10/28 */

/* FIH, Neo Chen, 2010/10/27, sc668 keypad LEDs{ */
static struct platform_device sc668_keypad_leds = {
	.name   = "keypad-leds",
	.id = -1,
};
/* } FIH, Neo Chen, 2010/10/27 */

/* FIH, Square, SC668, 2011/05/02 { */
static struct sc668_platform_data sc668_data = {
	.sc668_enable_pin = 30,
	.sc668_pwm_pin = 97,
};
/* }  FIH, Square, SC668, 2011/05/02 */

/* StanleyCheng, [DMP_porting] { */
#define TOUCH_RST 23  // tmp config for evb, the value should be 23
#define TOUCH_IRQ 89  // tmp config for evb, the value should be 89
#ifdef CONFIG_FIH_FXX
struct st1332_i2c_platform_data st1332_device = {
    .abs_x_min= 0,
    .abs_x_max= 320,
    .abs_y_min= 0,
    .abs_y_max= 480,
...
#endif
/* StanleyCheng, [DMP_porting] } */
static struct i2c_board_info i2c_devices_MT9P111[] = {
	{
		I2C_BOARD_INFO("mt9p111", 0x78 >> 1),
	}
};

static struct i2c_board_info i2c_devices_S5K5CAG[] = {
	{
		I2C_BOARD_INFO("s5k5cag", 0x3C),
	}
};
static struct i2c_board_info i2c_devices_HM0357[] = {
	{
		I2C_BOARD_INFO("hm0357", 0x61 >> 1),
	}
};

static struct i2c_board_info i2c_devices_STMPE1601[] = {
   {
	I2C_BOARD_INFO("stmpe1601-keypad", 0x40),
	.type		= "stmpe1601-keypad",
	.irq		= MSM_GPIO_TO_INT(91),
...
static struct i2c_board_info i2c_devices[] = {
	/* FIH, Square, SC668, 2011/05/02 { */	
	{
		I2C_BOARD_INFO("sc668", 0xE0 >> 1),//0x70
		.platform_data = &sc668_data,
	},
	/* }  FIH, Square, SC668, 2011/05/02 */	
...
#endif
/* FIH, Eric Wang, 2011/05/04 { */	
#ifdef CONFIG_INPUT_LPSENSOR
	{
		I2C_BOARD_INFO("ALS1", 0x90>>1),//0x48
	},
	{
		I2C_BOARD_INFO("ALS2", 0x92>>1),//0x49
	},
	{
		I2C_BOARD_INFO("PS1", 0xF0>>1),//0x78
	},
	{
		I2C_BOARD_INFO("PS2", 0xF2>>1),//0x79
	},
	{
		I2C_BOARD_INFO("ARA", 0x18>>1),//0x0c
	},
#endif
/* }  FIH, Eric Wang, 2011/05/04{ */		
...
#ifdef CONFIG_FIH_FXX
    {
        I2C_BOARD_INFO("st1332", 0x54),
        .irq            = MSM_GPIO_TO_INT(TOUCH_IRQ),
        .platform_data  = &st1332_device,
    },
#endif
/* StanleyCheng, [DMP_porting] { */
...
#endif
//Div2-D5-Peripheral-FG-TouchPorting-00+]
/* Thomas,porting,register i2c devices for gsensor/ecompass,2011,5,4 */
#ifdef CONFIG_FIH_FXX
        {
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k5cag_data = {
	.sensor_name    = "s5k5cag",
	.sensor_reset   = 0, // 
	.sensor_pwd     = 31, // FOR MQ4 is 1 but DMQ is 31 Orz..
	.vcm_pwd        = 0, // NA
	.vcm_enable     = 0, // NA
	.pdata          = &msm_camera_device_data,
...
static struct msm_camera_sensor_info msm_camera_sensor_mt9p111_data = {
	.sensor_name    = "mt9p111",
	.sensor_reset   = 0,
	.sensor_pwd     = 31,//97 for FN6.31 for DMP.
	.vcm_pwd        = 0,
	.vcm_enable     = 0,
	.pdata          = &msm_camera_device_data,
...
static struct msm_camera_sensor_info msm_camera_sensor_hm0357_data = {
	.sensor_name    = "hm0357",
	.sensor_reset   = 0,
	.sensor_pwd     = 77,//97 for FN6.77 for DMP.
	.vcm_pwd        = 0,
	.vcm_enable     = 0,
	.pdata          = &msm_camera_device_data,
...
#ifdef CONFIG_FIH_FXX
#ifdef CONFIG_SPI_GPIO
#define FIH_SPI_MOSI 132
#define FIH_SPI_MISO 131 //no use
#define FIH_SPI_CLK 101
#define FIH_SPI_CS 102
#define FIH_LCM_RST 103

static unsigned spi_gpio_config_input[] = {
    GPIO_CFG(FIH_SPI_CLK, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),  /* clk */
    GPIO_CFG(FIH_SPI_CS, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),  /* cs */
    GPIO_CFG(FIH_SPI_MOSI, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA),  /* mosi *///porting_ili
//	GPIO_CFG(FIH_SPI_MISO, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),  /* miso */
//    GPIO_CFG(30, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA),  /* mosi *///porting_ili
};
...
static int __init spi_gpio_init_fake(void)
{
    ...
    for (pin = 0; pin < ARRAY_SIZE(spi_gpio_config_input); pin++) {
        ...
        if (rc) {
			printk(KERN_ERR "%s: gpio_tlmm_config(%d)(%#x)=%d failed\n", __func__, (spi_gpio_config_input[pin]&0x3ff0)>>4,spi_gpio_config_input[pin], rc);
			return -EIO;
        }else{
    ...
    rc = gpio_request(FIH_LCM_RST, "lcd_reset");
    if (rc){
		printk(KERN_ERR "%s: lcd_reset setting failed! rc = %d\n", __func__, rc);
		return -EIO;
    }

	gpio_direction_output(FIH_LCM_RST,1);
	mdelay(500);
	printk(KERN_INFO "%s: (%d) gpio_read = %d\n", __func__, FIH_LCM_RST,gpio_get_value(FIH_LCM_RST));

	gpio_direction_output(FIH_LCM_RST,0);
	mdelay(500);
	printk(KERN_INFO "%s: (%d) gpio_read = %d\n", __func__, FIH_LCM_RST,gpio_get_value(FIH_LCM_RST));
    ...
    gpio_free(FIH_LCM_RST);

	/* spi-related gpio */
	rc = gpio_request(FIH_SPI_CLK, "gpio_spi");
	if (rc){
    	printk(KERN_ERR "%s: msm spi setting failed! rc = %d\n", __func__, rc);
		return -EIO;
	}
        rc = gpio_request(FIH_SPI_CS, "gpio_spi");
	if (rc){
        printk(KERN_ERR "%s: msm spi setting failed! rc = %d\n", __func__, rc);
		return -EIO;
	}

	rc = gpio_request(FIH_SPI_MOSI, "gpio_spi");
	if (rc){
        printk(KERN_ERR "%s: msm spi setting failed! rc = %d\n", __func__, rc);
		return -EIO;
	}

	gpio_direction_output(FIH_SPI_CLK,1);
    printk(KERN_INFO "%s: (101) gpio_read = %d\n", __func__, gpio_get_value(FIH_SPI_CLK));
	gpio_direction_output(FIH_SPI_CS,1);
    printk(KERN_INFO "%s: (102) gpio_read = %d\n", __func__, gpio_get_value(FIH_SPI_CS));
	gpio_direction_output(FIH_SPI_MOSI,1);	

	gpio_free(FIH_SPI_CLK);
    gpio_free(FIH_SPI_CS);
    gpio_free(FIH_SPI_MOSI);               
...
static struct spi_board_info lcdc_spi_devices[] = {
        {
                ...
                .chip_select = 0,
                .controller_data = (void *) FIH_SPI_CS,
        		.mode           = SPI_MODE_3,
        },
};
...
static struct platform_device msm_batt_device = {
	.name 		    = "goldfish-battery",   /*MayLi, add DominoP battery driver, 2011.05.04 */
	.id		    = -1,
...
// Bluetooth, KennyChu +++
#ifdef CONFIG_RFKILL
static struct platform_device tiwlan_rfkill = {
	        .name = "tiwlan_rfkill",
		        .id = -1,
};
#endif
...
// FIH, Vince 2010/11/09+++
	GPIO_CFG(29, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA),
	//gpio_direction_input(rd->cam_sw_t_pin);  
	GPIO_CFG(20, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA),
	//gpio_direction_input(rd->cam_sw_f_pin); 
// FIH, Vince 2010/11/09---
};
static uint32_t DPD_keypad_gpio_table[] = {

/* Config GPIO for keypad         */
	GPIO_CFG(94,  0, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA), /* hook switch*/
	GPIO_CFG(40,  0, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA), /* headset plug*/
	GPIO_CFG(28,  0, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA), /* Volume Up Key    */
	GPIO_CFG(19,  0, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA), /* Volume Down Key  */
...
static uint32_t DMQ_keypad_gpio_table[] = {
...
static uint32_t AI1_keypad_gpio_table[] = {

/* Config GPIO for keypad         */
        GPIO_CFG(41,  0, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA), /* backcover    */
	GPIO_CFG(94,  0, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA), /* hook switch*/
	GPIO_CFG(40,  0, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA), /* headset plug*/
	GPIO_CFG(88,  0, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA), /*key F2*/
	GPIO_CFG(17,  0, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA), /*key F1*/
	GPIO_CFG(28,  0, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA), /* Volume Up Key    */
	GPIO_CFG(19,  0, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA), /* Volume Down Key  */
// FIH, Vince 2010/11/09---
};
static uint32_t FAD_keypad_gpio_table[] = {
...
static void config_keypad_gpios(void)
{
   ...
   printk("board: productHWID=%d",productHWID) ;
   if(productHWID==Project_DPD || productHWID==Project_DMP || productHWID==Project_DP2)
   {
   	printk("board: CONFIGURE DPD_keypad_gpio_table \n") ;
   	
   	config_gpio_table(DPD_keypad_gpio_table,
		ARRAY_SIZE(DPD_keypad_gpio_table));
   }
   else if( productHWID==Project_DMQ || productHWID==Project_DQE || productHWID==Project_DQD)
   {
   	printk("board: CONFIGURE DMQ_keypad_gpio_table \n") ;
   	config_gpio_table(DMQ_keypad_gpio_table,
		ARRAY_SIZE(DMQ_keypad_gpio_table));
   }
   else if(productHWID==Project_AI1D || productHWID==Project_AI1S)
   {
   	config_gpio_table(AI1_keypad_gpio_table,
		ARRAY_SIZE(AI1_keypad_gpio_table));
   }
   else if(productHWID==Project_FAD)
   {
   	config_gpio_table(FAD_keypad_gpio_table,
		ARRAY_SIZE(FAD_keypad_gpio_table));
   }
   else
   {
...
/* FIH, Debbie Sun, 2010/09/30 { */
/* add UART2DM for UART */
#ifdef CONFIG_FIH_FXX
	&msm_device_uart_dm12,  //Debbie, 2010/09/30
#endif
/* } FIH, Debbie Sun, 2010/09/30 */
	&msm_device_smd,
	&msm_device_dmov,
...
static struct platform_device *devices_HM0357[] __initdata = {
	&msm_camera_sensor_hm0357,
};
static struct platform_device *devices_MT9P111[] __initdata = {
	&msm_camera_sensor_mt9p111,
};


static struct platform_device *devices_S5K5CAG[] __initdata = {
...
/* FIH, SquareCHFang, 2011/05/03 { */
/* add pull down confi for sdc1 */
...
static struct sdcc_gpio sdcc_cfg_data[] = {
	{
		...
		.sleep_cfg_data = sdc1_sleep_cfg_data,  /* FIH, SquareCHFang, 2011/05/03 */
	},
	{
...
static uint32_t msm_sdcc_setup_power(struct device *dv, unsigned int vdd)
{
	...
	printk(KERN_ERR "%s: id: %d , vdd: %d \n", __func__, pdev->id, vdd);

	// This function only for SD Card, the dev is 1	
	if( pdev->id != 1)
	{
		printk(KERN_ERR "%s: skip, because only for id is 1. (SD Card) \n", __func__);
		return 0;
	}
	
	if(vreg_mmc == NULL)
	{
		printk(KERN_ERR "%s: skip, because not find SD Card power register.\n", __func__);
		return 0;
	}
...
static uint32_t msm_sdcc_setup_power_wl1271(struct device *dv, unsigned int vdd)
{
...
static struct mmc_platform_data msm7x27_wl1271_sdcc_data = {
        .ocr_mask       = MMC_VDD_28_29,
        .translate_vdd  = msm_sdcc_setup_power_wl1271,
        .mmc_bus_width  = MMC_CAP_4_BIT_DATA | MMC_CAP_TIWLAN_WL1271 ,
...
static void __init msm7x2x_init_mmc(void)
{
	...
	/*
       Monky: Change to use proper correct ID
    */
...
/* FIH, SimonSSChang TI WIFI */
#ifdef CONFIG_TIWLAN_SUPPORT
		msm_add_sdcc(2, &msm7x27_wl1271_sdcc_data);
#endif
/* FIH, SimonSSChang TI WIFI */
#else
...
static void ti_wifi_power_init(void)
{
    int rc; 
    int productHWID=0 ;
    ...
    printk(KERN_INFO "ti_wifi_power_init() productHWID = %d\n", productHWID);
    
    if(productHWID == Project_AI1D || productHWID == Project_AI1S || productHWID == Project_DQE || productHWID == Project_DQD)
    {
        printk("ti_wifi_power_init() config gp4, GPIO35, GPIO27");
	if (vreg_gp4 == NULL) {
		vreg_gp4 = vreg_get(NULL, "gp4");
		if (IS_ERR(vreg_gp4)) {
			printk(KERN_ERR "%s: vreg_get(%s) failed (%ld)\n",
				__func__, "gp4", PTR_ERR(vreg_gp4));
			return;
		}
		rc = vreg_set_level(vreg_gp4, 1800);
		if (rc) {
			printk(KERN_ERR "%s: GP4 set_level failed (%d)\n",
				__func__, rc);
		}
    }
    
    printk(KERN_INFO "set vreg_gp4 is 1.8V\n");
	rc = vreg_enable(vreg_gp4);
	if (rc) {
		printk(KERN_ERR "%s: GP4 enable failed (%d)\n",
			 __func__, rc);
	}

    //enable WL_EN
    mdelay(50);
    printk(KERN_INFO "WIFI: pull up WL_EN pin 35\n");
    gpio_direction_output(35,0);    
    
    
    printk(KERN_INFO "BT: pull up BT_EN pin 27\n");    
    gpio_direction_output(27,0);        
    }
    else
    {
        printk("ti_wifi_power_init() NO config gp4, GPIO35, GPIO27\n");
    }     
}
#endif
/* FIH, SimonSSChang TI WIFI */
...
/* } FIH, Vince Liu, @2010/12/07 */
	...
	if(fih_read_new_hwid_mech_from_orighwid())
        {
   	     project_HWID = fih_read_product_id_from_orighwid() ;      
        }
        if(project_HWID==Project_DPD || project_HWID==Project_DMP || project_HWID==Project_AI1D || project_HWID==Project_AI1S || project_HWID==Project_DP2)
	{
		platform_add_devices(devices_MT9P111, ARRAY_SIZE(devices_MT9P111));
		platform_add_devices(devices_HM0357, ARRAY_SIZE(devices_HM0357));
	}
	else if(project_HWID==Project_DMO ||project_HWID==Project_DMT)
	{
		platform_add_devices(devices_S5K5CAG, ARRAY_SIZE(devices_S5K5CAG));
		platform_add_devices(devices_HM0357, ARRAY_SIZE(devices_HM0357));
	}
	else if(project_HWID==Project_DMQ || project_HWID==Project_DQE||project_HWID==Project_DQD)
	{
		platform_add_devices(devices_S5K5CAG, ARRAY_SIZE(devices_S5K5CAG));
	}
        else if(project_HWID==Project_FAD)
	{
		flash_mt9p111.flash_type = MSM_CAMERA_FLASH_NONE;	    
        	printk( "set mt9p111 flash type as none for FAD\r\n" );
		platform_add_devices(devices_MT9P111, ARRAY_SIZE(devices_MT9P111));
	}
	else
	{
		printk("Warning: Platform No cammera sensor is selected!!!\n") ;
	}
#ifdef CONFIG_MSM_CAMERA
...
/* FIH, Chandler Kang, 2009/05/18 } */
	...
	if(fih_read_new_hwid_mech_from_orighwid())
        ...
        //printk("#######HWID = %d!",project_HWID);
	i2c_register_board_info(0, i2c_devices, ARRAY_SIZE(i2c_devices));
	if(project_HWID==Project_DPD || project_HWID==Project_DMP || project_HWID==Project_AI1D || project_HWID==Project_AI1S || project_HWID==Project_DP2)
	{
		i2c_register_board_info(0, i2c_devices_MT9P111, ARRAY_SIZE(i2c_devices_MT9P111));
		i2c_register_board_info(0, i2c_devices_HM0357, ARRAY_SIZE(i2c_devices_HM0357));
	}
	else if(project_HWID==Project_DMO ||project_HWID==Project_DMT)
	{
		printk("register i2c camera sensor\n") ;
		i2c_register_board_info(0, i2c_devices_S5K5CAG, ARRAY_SIZE(i2c_devices_S5K5CAG));
		i2c_register_board_info(0, i2c_devices_HM0357, ARRAY_SIZE(i2c_devices_HM0357));
	}
	else if(project_HWID==Project_DMQ || project_HWID==Project_DQE||project_HWID==Project_DQD)
	{
	...
	{
		printk("Warning: No cammera sensor is selected!!!\n") ;
	}

//register keypad driver
	if(project_HWID==Project_AI1D || project_HWID==Project_AI1S || project_HWID==Project_DMQ || project_HWID==Project_DQE || project_HWID==Project_DQD)
	{
		i2c_register_board_info(0, i2c_devices_STMPE1601, ARRAY_SIZE(i2c_devices_STMPE1601));
	}
