...
/* } FIH, SimonSSChang, 2009/12/15 */
	...
	if (allow[MSM_PM_SLEEP_MODE_POWER_COLLAPSE] ||
		...
		if(dorm_fifo==NULL)
		{  
			dorm_fifo = filp_open("/data/radio/dorm_fifo", O_RDWR, 0);
			dorm_input[0] = '1';
...
#define FLUSH_CACHE_SIZE    0x40000 // 256KB
    ...
    if (panic_enter_download)
    {
        ...
        *(uint32_t *)ptr = 0x55AA55AA;
        ...
        memcpy((void*)((MSM_PLOG_BASE + 0x80000) - FLUSH_CACHE_SIZE), (void*)ptr, FLUSH_CACHE_SIZE);
    }
    /* FIH, Paul Huang, 2009/10/20 } */
	...
	(struct notifier_block *this, unsigned long code, void *_cmd)
{
	if ((code == SYS_RESTART) && _cmd) {
		...
		if (!strcmp(cmd, "bootloader")) {
			...
			restart_reason = 0x444C4F41; // A11_REBOOT_REASON_TO_DL_MODE Reboot to modem download mode
