 ...
 *
 * @brief
 *	An update process progress indicator.
 *
 *	A glue function that needs to be implemented by the customer.
 *	Indicates how much of the differential data has been merged with the original. 
 *
 *	Acts as a callback, the update procedure calls this function occasionally, and by that
 *	return control to the application. the application should return from the callback
 *	gracefully in order to finish the update correctly.
 *
 *	@param pbUserData	Any user data structure, that may be useful for the user
 *						implementation, if not needed set to NULL.
 *						The calling function supply you with the user data,
 *						previously supplied in the RB_FileSystemUpdate.
 *
 *	@param uPercent		The current progress given in percentage
 *
 ************************************************************ 
 */
 ...
 *
 * @brief
 *	Get the Delta either as a whole or in block pieces.
 *	this function is used if the strDeltaPath parameter in 
 *	RB_FileSystemUpdate function is set to NULL. 
 *
 *	@param pbBuffer				pointer to user buffer.
 *
 *	@param dwStartAddressOffset	Offset from start of delta file.
 *
