...
#define FS_U_RAM_SIZE	10*1024*1024
...
#define DBG_MSG(...)    \
    ...
    {                   \
        if(fp_pipe)     \
        {               \
            fprintf(fp_pipe, "FOTA:" __VA_ARGS__); \
            fflush(fp_pipe);    \
        }   \
        printf("FOTA:" __VA_ARGS__); \
    }while(0)
	...
	const		mode_t mode)
{
	...
	DBG_MSG(" mkdir result: %d errno: %d\n", ret, errno);
	...
	const unsigned short*	strToPath)
{
	...
	DBG_MSG ("%s: %s -> %s ", __func__, path1, path2);
	if (!strFromPath || !strToPath)
	{
		DBG_MSG("NULL file name find. Abort.\n");
	...
	if (fileType == FT_SYMBOLIC_LINK)
	{
		...
		if (ret < 0)
		{
			DBG_MSG(" readlink failed with return value: %d\n",ret);
			return -2;
		}
	...
	if (!fp1)
	{
		DBG_MSG(" Open %s ENOENT %d\n", path1, errno);
		DBG_MSG("Open %s failed. Abort.\n", path1);
	...
	while( (readCount = fread(buf, 1, BUFFER_SIZE, fp1))> 0)
	{
		...
		{
			DBG_MSG(" read %d, but write %d, abort.\n", readCount, writeCount);
	...
	const unsigned short*	strPath)
{
	...
	DBG_MSG ("%s: %s\n", __func__, path);
	ret = unlink(path);
	DBG_MSG (" unlink value: %d, errno: %d\n", ret, errno);
	if (ret == 0)
		return S_RB_SUCCESS;
	...
	long*					pwHandle)
{
	...
	DBG_MSG("%s: Path:%s | Mode:", __func__, path);
	mode = get_mode(wFlag);
	...
	*pwHandle = open(path, mode);
	...
	{
		*pwHandle = 0;
		DBG_MSG(" First open() with error %d\n", errno);
		if (wFlag == ONLY_R)
			return E_RB_OPENFILE_ONLYR;
		...
		if ((wFlag != ONLY_R) && (errno == ENOENT))
		{
			...
			*pwHandle = open(path, mode);
			...
			{
				*pwHandle = 0;
				DBG_MSG(" After successful creating folder, fail open() with error %d\n", errno);
				return E_RB_OPENFILE_WRITE;
			}
		}
 	}
	DBG_MSG(" Successful open() *pwHandle:%ld\n", *pwHandle);
	...
	unsigned long	dwSize)
{
	...
	DBG_MSG("%s: Handle:%ld , Pos:%ld , Size: %ld", __func__, wHandle,dwPosition,dwSize);
	...
	if (ret < 0)
	{
		DBG_MSG(" lseek failed with return value: %d\n",ret);
		return E_RB_WRITE_ERROR;
	}
	...
	if (ret < 0)
	{
		DBG_MSG(" Failed with return value: %d\n",ret);
		return E_RB_WRITE_ERROR;
	}
	DBG_MSG(" Bytes Write: %d\n",ret);

	ret = fsync(wHandle);
	if (ret < 0)
	{
		DBG_MSG(" fsync Failed with return value: %d\n",ret);
		return E_RB_WRITE_ERROR;
	}
	DBG_MSG(" fsync after write: %d\n",ret);
	...
	unsigned long	dwSize)
{
	...
	if (ret < 0)
	{
		DBG_MSG(" read failed with return value: %d\n",ret);
		return E_RB_READ_ERROR;
	}
	
	DBG_MSG(" Bytes Read: %d\n",ret);
	...
	long	wHandle)
{
	...
	DBG_MSG("%s: %ld ", __func__, wHandle);

	ret = lseek(wHandle, 0, SEEK_END);

	if (ret == -1)
	{
		DBG_MSG(" lseek errno: %d\n", errno);
		return E_RB_READFILE_SIZE;
	}
	DBG_MSG("Returning Size = 0x%lx\n",ret);
	...
	unsigned short*	pLinkName)
{
	...
	{
		DBG_MSG("cannot unlink file type %d\n", fileType);
	...
	if(ret < 0)
	{
		DBG_MSG(" unlink failed with return value: %d\n",ret);
		return E_RB_FAILURE;
	}
	DBG_MSG(" unlink with return value: %d\n",ret);
	...
	unsigned short*	pReferenceFileName)
{
	...
	{
		DBG_MSG(" get file type failed with file type: %d value %d\n", fileType, ret);
	...
	if (ret != 0)
	{
		DBG_MSG(" symlink failed with return value: %d, errno: %d\n", ret, errno);
	...
	DBG_MSG(" symlink with return value: %d\n",ret);
	...
	unsigned short*	pReferenceFileName)
{
	...
	{
		DBG_MSG(" not same linked path - linkedpath[%s] pReferenceFileName[%s]\n", linkedpath, refPath);
		return E_RB_FAILURE;
	}
	DBG_MSG(" same linked path\n");
	...
	enumFileType*	fileType)
{
	...
	if (ret < 0)
	{
		DBG_MSG(" stat failed with return value: %d errno: %d\n", ret, errno);
		*fileType = FT_MISSING;
		return S_RB_SUCCESS;
	}
	...
	{
		DBG_MSG(" stat->st_mode = symbolic link file\n");
	...
	{
		DBG_MSG(" stat->st_mode = regular file\n");
...
char a2ch(int value)
{
	...
	switch(value)
	{
		...
		case '0':
			...
			break;
		default:
			DBG_MSG("Wrong attribute value: %d\n",value);
				...
				const unsigned char *ui8pAttribs)
{
	...
	{
		DBG_MSG("ui16pFilePath NULL [error]\n");
		return E_RB_BAD_PARAMS;
	}
	else if(NULL == ui8pAttribs)
	{
		DBG_MSG("ui8pAttribs NULL [error]\n");
		return E_RB_BAD_PARAMS;
	}
	...
	if(ret < 0)
	{	
			DBG_MSG(" stat failed with return value: %d\n",ret);
			return E_RB_FAILURE;
	}
	else
	{
		...
		if (S_ISDIR(sbuf.st_mode))
		{
			DBG_MSG(" stat->st_mode = directory\n");
		}
	}
	
	if(0 == ui32AttribSize)
	{
		DBG_MSG("ui32AttribSize 0\n");
		return S_RB_SUCCESS;
	}
	
	DBG_MSG("ui16pFilePath = %s\n", setFilePath);
	DBG_MSG("ui32AttribSize = %u\n", ui32AttribSize);
	DBG_MSG("ui8pAttribs = %s\n", ui8pAttribs);
	// debug start
	for(count=0;count<ATTRSIZE;count++)
	{
		DBG_MSG("ui8pAttribs[%d] = %c\n", count, ui8pAttribs[count]);
	}
	// debug end
	...
	// Set FileMode
	DBG_MSG("setFilePath = %s\n",setFilePath);
	DBG_MSG("setFileMode = %d\n", setFileMode);
	DBG_MSG("setFileMode = %o\n", setFileMode);
	DBG_MSG("setUserID = %d\n", setUserID);
	DBG_MSG("setGroupID = %d\n", setGroupID);
	
	// Set UserID,GroupID
	if( chown(setFilePath, setUserID, setGroupID) )
	{
		DBG_MSG("%s chown error\n", __func__);
		// debug start
		DBG_MSG("%s setUserID = %d\n", __func__, setUserID);
		DBG_MSG("%s setGroupID = %d\n", __func__, setGroupID);
		DBG_MSG("%s chown errno = %d\n", __func__, errno);
		// debug end
		return E_RB_FAILURE;
	}
...
#define DELTA_FILE "/data/data/com.fihtdc.dm/files/dlpkgfile"
...
int main(int argc, char **argv)
{
	...
	unsigned char *string_del = "/data/data/com.fihtdc.dm/files/dlpkgfile";
	...
	DBG_MSG("return value from RB_vRM_Update 0x%x\n", ret);
	DBG_MSG("pCustomerPartData.updated = rest = %d\n", rest);
	if (ret == 0)
		DBG_MSG("FOTA update success...\n");
