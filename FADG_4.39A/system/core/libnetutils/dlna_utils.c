...
/* Utilities for managing the DLNA digital media server daemon */
...
static const char DAEMON_NAME[]        = "dlna";
static const char DAEMON_PROP_NAME[]   = "init.svc.dlna";
static const char HOSTNAME_PROP_NAME[] = "net.hostname";
static const char DHCP_PROP_NAME_PREFIX[]  = "dlna";
static const int  NAP_TIME = 1;   /* wait for 1 second at a time */
                                  /* when polling for property values */
static char errmsg[100];
static const char P2P_INTERFACE_PREFIX[]  = "p2p";
...
/*
 * Start the dms daemon, and wait for it
 */
int dlna_do_request(const char *interface)
{
    ...
    char prefix[PROPERTY_VALUE_MAX * 2];

    if (memcmp(interface,
               P2P_INTERFACE_PREFIX,
               strlen(P2P_INTERFACE_PREFIX)) == 0) {
        strcpy(prefix, P2P_INTERFACE_PREFIX);
    } else {
        strcpy(prefix, interface);
    }
    ...
    //    snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:-h %s %s", DAEMON_NAME, prefix,
    //             prop_value, interface);
    //else
        snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:%s", DAEMON_NAME, prefix, interface);
    memset(prop_value, '\0', PROPERTY_VALUE_MAX);
    property_set(ctrl_prop, daemon_cmd);
    if (wait_for_property(daemon_prop_name, desired_status, 10) < 0) {
        snprintf(errmsg, sizeof(errmsg), "%s", "Timed out waiting for dms to start");
        return -1;
    }
 ...
 * Stop the dms daemon.
 */
int dlna_stop(const char *interface)
{
    char result_prop_name[PROPERTY_KEY_MAX];
    char daemon_prop_name[PROPERTY_KEY_MAX];
    char daemon_cmd[PROPERTY_VALUE_MAX * 2];
    ...
    snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s", DAEMON_NAME, prefix);
                    ...
                    uint32_t  *lease)
{
    ...
    snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:%s", DAEMON_NAME_RENEW, prefix, interface);
    memset(prop_value, '\0', PROPERTY_VALUE_MAX);
    property_set(ctrl_prop, daemon_cmd);
