...
#define FRAMEWORK_REBOOT_COUNT_TEXT     "Framework_Reboot_Count:"
...
#define FRAMEWORK_REBOOT_BIT	0x80000000UL
...
update_exception_counter(const char*  path)
{
	...
	unsigned int framework_reboot_count = 0;
	...
	if (exception_stat_fd < 0) 
	{
		...
		len += sprintf(&buf[len], FRAMEWORK_REBOOT_COUNT_TEXT " %u\n", framework_reboot_count);
	...
	if(exception_stat_fd >= 0)
	{
		...
		{
			sscanf(buf, "%*s %u %*s %u %*s %u %*s %u*s %u",
	   		&kernel_panic_count,
	   		&modem_panic_count,
	   		&power_cut_count,
	   		&framework_reboot_count,
			...
			if (power_on_cause & FRAMEWORK_REBOOT_BIT)
			{
			    framework_reboot_count++;
			}
