...
void start_device_log(void)
{
    ...
    #else
    strftime(stime, sizeof(stime), "%Y-%m-%d-%H:%M:%S", &now);
    fd = unix_open("/data/alog_adb", O_WRONLY | O_CREAT | O_APPEND, 0640);
    #endif
    //Div6D1-JL-AdbRootPorting-00+}
    ...
    fprintf(stderr,"--- adb starting (pid %d) ---\n", getpid());
    #else
    fprintf(stderr,"--- adb starting (pid %d) (time %s) ---\n", getpid(), stime);
    #endif
    //Div6D1-JL-AdbRootPorting-00+}
...
int adb_main(int is_daemon, int server_port)
{
    ...
    */
    ...
    if (strcmp(value, "1") != 0) {
        ...
        {
            secure = 1;
            if((fp = fopen("/sys/devices/virtual/usb_composite/adb/root", "r")))
            {
                if (fgets(root_state, sizeof(root_state), fp))
                {
                    if (!strncmp(root_state, "1", 1))
                    {
                        secure = 0;
                    }
                }
